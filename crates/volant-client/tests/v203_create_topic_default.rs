//! v0.203: Rust Client CreateTopic default-partitions named helper.
...
struct SeenCreateTopic {
    name: String,
    partitions: u32,
}

struct CreateTopicStub {
    addr: String,
    seen: Arc<Mutex<Vec<SeenCreateTopic>>>,
    server: tokio::task::JoinHandle<()>,
}

impl CreateTopicStub {
    async fn boot() -> Self {
    ...
    fn seen(&self) -> Vec<SeenCreateTopic> {
        self.seen.lock().expect("seen").clone()
    }
}

impl Drop for CreateTopicStub {
    fn drop(&mut self) {
        self.server.abort();
    }
}
                ...
                Ok(Some(frame)) => {
                    ...
                    let response = match req {
                        Request::CreateTopic {
                            ...
                            seen.lock().expect("seen").push(SeenCreateTopic {
                                name: name.clone(),
                                partitions,
                            });
...
#[tokio::test]
async fn create_topic_default_encodes_partitions_one() {
    let stub = CreateTopicStub::boot().await;
    let client = Client::connect_addr(&stub.addr).await.expect("connect");
    let topic_id = client
        .create_topic_default("events")
        .await
        .expect("create_topic_default");
