//! v0.96: Rust Client Metadata / ListMembers retry on transient errors.
        ...
        let server = tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    let _ = serve_stub(stream, metadata, lists, queued).await;
                });
            }
        });
...
#[tokio::test]
async fn retries_list_members_timeout_then_ok() {
    let stub = MetaStub::boot([TIMEOUT, 0]).await;
    let client = connect(&stub.addr, 2, 0).await;
    let members = client.list_members().await.expect("list");
    assert_eq!(members.generation, 1);
    assert!(members.brokers.is_empty());
    assert!(members.live.is_empty());
    assert_eq!(stub.list_rpcs(), 2);
}

#[tokio::test]
async fn not_found_is_not_retried() {
    let stub = MetaStub::boot([NOT_FOUND, 0]).await;
    let client = connect(&stub.addr, 2, 0).await;
    let err = client.metadata().await.expect_err("not found");
...
async fn exhausted_retries_surface_timeout() {
    let stub = MetaStub::boot([TIMEOUT, TIMEOUT, TIMEOUT]).await;
    let client = connect(&stub.addr, 2, 0).await;
    let err = client.metadata().await.expect_err("exhausted");
    assert_eq!(surfaced_code(&err), Some(TIMEOUT));
    assert_eq!(stub.metadata_rpcs(), 3);
}
