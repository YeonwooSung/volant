//! v0.224: Client::join_group_with_instance retries error 9.
        ...
        let server = tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    let _ = serve_stub(stream, jn, queued).await;
                });
            }
        });
...
#[tokio::test]
async fn join_retries_error_9_then_ok() {
    let stub = JoinGroupStub::boot([REBALANCE, 0]).await;
    let client = connect(&stub.addr, 1, 0).await;
    let result = client
        .join_group_with_instance("g", "m-rejoin", 10_000, vec!["t".into()], "")
        .await
        .expect("join after error 9 retry");
...
#[tokio::test]
async fn join_default_max_retries_does_not_retry_9() {
    let stub = JoinGroupStub::boot([REBALANCE, 0]).await;
