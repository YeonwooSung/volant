//! v0.154: Rust Client OffsetFetch topic filter + metadata.
...
#[tokio::test]
async fn fetch_offsets_for_topic_keeps_metadata() {
    let stub = OffsetFetchStub::boot().await;
    let client = Client::connect_addr(&stub.addr).await.expect("connect");
    let rows = client
        .fetch_offsets_for_topic("g", "t")
        .await
        .expect("fetch_offsets_for_topic");
    assert_eq!(
        rows,
        vec![OffsetFetchEntry {
            topic: "t".into(),
            partition: 0,
            offset: 5,
            metadata: "consumer-1".into(),
...
#[tokio::test]
async fn fetch_offsets_empty_returns_both_topics() {
    let stub = OffsetFetchStub::boot().await;
    let client = Client::connect_addr(&stub.addr).await.expect("connect");
    let rows = client
        .fetch_offsets("g", vec![])
    ...
    assert_eq!(rows, two_topic_entries());
    assert_eq!(stub.seen_entries(), vec![Vec::<OffsetEntry>::new()]);
}
