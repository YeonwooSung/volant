//! v0.174: Rust Client create_scram_user_default iterations-0 wrapper.
...
struct ScramStub {
    addr: String,
    seen: Arc<Mutex<Vec<SeenCreateScram>>>,
...
impl ScramStub {
    ...
    fn seen(&self) -> Vec<SeenCreateScram> {
                ...
                Ok(Some(frame)) => {
                    ...
                    let response = match req {
                        Request::CreateScramUser {
                            ...
                            seen.lock().expect("seen").push(SeenCreateScram {
                                username,
                                password,
                                iterations,
                            });
                            Response::CreateScramUser { error_code: 0 }
                        }
...
#[tokio::test]
async fn create_scram_user_default_encodes_iterations_zero() {
    let stub = ScramStub::boot().await;
    let client = Client::connect_addr(&stub.addr).await.expect("connect");
    client
        .create_scram_user_default("alice", "s3cret")
        .await
        .expect("create_scram_user_default");
    assert_eq!(
        stub.seen(),
        vec![SeenCreateScram {
            username: "alice".into(),
