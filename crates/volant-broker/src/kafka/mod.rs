...
//! EndQuorumEpoch v1 (key 54 reject; not KRaft quorum epoch),
...
//! StreamsGroupHeartbeat v0 (key 88 reject; not KIP-1071 streams groups),
//! StreamsGroupDescribe v0 (key 89 reject; not KIP-1071 streams groups),
...
pub enum ApiKey {
    ...
    BeginQuorumEpoch = 53,
    /// EndQuorumEpoch (always flexible; v1 only). Honest reject:
    /// not a KRaft quorum; does not wrap openraft RequestVote,
    /// Vote 52, or BeginQuorumEpoch 53. Official v0 is classic —
    /// not advertised.
    EndQuorumEpoch = 54,
    ...
    /// StreamsGroupHeartbeat (always flexible; v0 only). Honest
    /// reject: not KIP-1071 streams groups. Does not persist and
    /// does not wrap Heartbeat 12 / ConsumerGroupHeartbeat 68 /
    /// ShareGroupHeartbeat 76 / `GroupCoordinator::heartbeat`.
    /// Official validVersions is 0–1 (v1 = TopologyDescriptionRequired
    /// / KIP-1331); Volant advertises v0 only.
    StreamsGroupHeartbeat = 88,
    /// StreamsGroupDescribe (always flexible; v0 only). Honest reject:
    /// not KIP-1071 streams groups. Does not wrap `describe_group` /
    /// ConsumerGroupDescribe 69 / DescribeGroups 15 / ShareGroupDescribe
    /// 77. Official validVersions is 0–1 (v1 adds
    /// IncludeTopologyDescription / KIP-1331); Volant advertises 0 only.
    ...
    /// AlterShareGroupOffsets (always flexible; v0 only). Honest
    /// reject: not KIP-932 share offsets. Does not persist and does
    /// not wrap OffsetCommit 8 / DescribeShareGroupOffsets 90.
    /// Official validVersions is 0 only.
...
pub const SUPPORTED_APIS: &[(ApiKey, i16, i16)] = &[
    ...
    (ApiKey::StreamsGroupHeartbeat, 0, 0),
...
mod tests {
    ...
    #[test]
    fn supported_apis_includes_end_quorum_epoch_54() {
        assert!(SUPPORTED_APIS.len() >= 90);
        assert!(SUPPORTED_APIS
            .iter()
            .any(|(k, min, max)| *k == ApiKey::EndQuorumEpoch && *min == 1 && *max == 1));
        assert_eq!(ApiKey::from_i16(54), Some(ApiKey::EndQuorumEpoch));
        assert_eq!(ApiKey::from_i16(52), Some(ApiKey::Vote));
        assert_eq!(ApiKey::from_i16(53), Some(ApiKey::BeginQuorumEpoch));
        assert_eq!(ApiKey::from_i16(55), Some(ApiKey::DescribeQuorum));
    }
    ...
    #[test]
    fn supported_apis_includes_streams_group_heartbeat_88() {
        assert!(SUPPORTED_APIS.len() >= 85);
        assert!(SUPPORTED_APIS.iter().any(|(k, min, max)| {
            *k == ApiKey::StreamsGroupHeartbeat && *min == 0 && *max == 0
        }));
    ...
    #[test]
    fn supported_apis_includes_streams_group_describe_89() {
        assert!(SUPPORTED_APIS.len() >= 85);
        assert!(SUPPORTED_APIS
            .iter()
            .any(|(k, min, max)| { *k == ApiKey::StreamsGroupDescribe && *min == 0 && *max == 0 }));
    ...
    #[test]
    fn supported_apis_includes_alter_share_group_offsets_91() {
        assert!(SUPPORTED_APIS.len() >= 85);
        assert!(SUPPORTED_APIS.iter().any(|(k, min, max)| {
            *k == ApiKey::AlterShareGroupOffsets && *min == 0 && *max == 0
        }));
        assert_eq!(ApiKey::from_i16(91), Some(ApiKey::AlterShareGroupOffsets));
        assert_eq!(
            ApiKey::from_i16(90),
            Some(ApiKey::DescribeShareGroupOffsets)
        );
