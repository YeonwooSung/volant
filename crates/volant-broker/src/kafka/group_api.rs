...
/// ShareGroupHeartbeat v1 (key 76). Always flexible.
///
/// Official `ShareGroupHeartbeatRequest.json` / `Response.json`.
/// Not KIP-932: parse and reject **42** `INVALID_REQUEST`
/// (`not KIP-932 share group`). Does not call
/// `GroupCoordinator::heartbeat` and does not wrap classic Heartbeat 12
/// or ConsumerGroupHeartbeat 68. Official v0 was removed in Kafka 4.1.
pub(crate) fn encode_share_group_heartbeat(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let group_id = parse_share_group_heartbeat_request(src);
    ...
    write_share_group_heartbeat(out, denied);
}

fn parse_share_group_heartbeat_request(src: &mut impl Buf) -> String {
    // Official v1 (flex, `ShareGroupHeartbeatRequest.json`):
    // GroupId compact string, MemberId compact string, MemberEpoch i32,
    // RackId compact nullable string, SubscribedTopicNames compact
    // nullable array of compact string, tagged.
    let group_id = get_compact_string(src).unwrap_or_default();
    let _ = get_compact_string(src);
    if src.remaining() >= 4 {
        let _member_epoch = src.get_i32();
...
fn write_share_group_heartbeat(out: &mut BytesMut, denied: bool) {
    // Official ShareGroupHeartbeatResponse.json v1:
    // throttleTimeMs, errorCode, errorMessage, memberId, memberEpoch,
    // heartbeatIntervalMs, assignment (nullable struct; 0 = null), tagged.
    out.put_i32(0); // throttleTimeMs
    if denied {
        ...
        out.put_i16(KafkaErrorCode::InvalidRequest.as_i16());
        put_compact_nullable_string(out, Some("not KIP-932 share group"));
    }
    put_compact_nullable_string(out, None); // memberId
    out.put_i32(-1); // memberEpoch
    out.put_i32(0); // heartbeatIntervalMs
...
/// ReadShareGroupState v0 (key 84). Always flexible.
///
/// Official `ReadShareGroupStateRequest.json` / `Response.json`.
/// Not KIP-932: parse and reject per-partition **42** `INVALID_REQUEST`
/// (`not KIP-932 share state`). Does not persist share state and does
/// not wrap OffsetFetch / OffsetCommit / InitializeShareGroupState.
/// Official response has no throttle and no top-level error — echo
/// TopicId + Partition with the per-partition code, StateEpoch **0**,
/// StartOffset **-1**, empty `StateBatches[]`. Unparseable body →
/// empty `Results[]`.
pub(crate) fn encode_read_share_group_state(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let (group_id, topics) = parse_read_share_group_state_request(src);
    ...
    write_read_share_group_state(out, &topics, denied);
}

fn parse_read_share_group_state_request(
    src: &mut impl Buf,
) -> (String, Vec<([u8; 16], Vec<i32>)>) {
    // Official v0 (flex, `ReadShareGroupStateRequest.json`):
    // GroupId compact string, Topics[] { TopicId uuid, Partitions[] {
    // Partition i32, LeaderEpoch i32, tagged }, tagged }, tagged.
    // Echo TopicId + Partition; discard LeaderEpoch.
    let group_id = get_compact_string(src).unwrap_or_default();
    let mut topics = Vec::new();
    match get_compact_array_len(src) {
        Ok(Some(n)) => {
...
fn write_read_share_group_state(
    out: &mut BytesMut,
    topics: &[([u8; 16], Vec<i32>)],
    denied: bool,
) {
    // Official ReadShareGroupStateResponse.json v0:
    // Results[] { TopicId uuid, Partitions[] { Partition i32,
    // ErrorCode i16, ErrorMessage compact nullable string,
    // StateEpoch i32, StartOffset i64, StateBatches[] empty, tagged },
    // tagged }, tagged. No throttleTimeMs. No top-level error.
    put_compact_array_len(out, topics.len());
    for (topic_id, parts) in topics {
        put_uuid(out, topic_id);
        ...
        for partition in parts {
            ...
            out.put_i32(0); // StateEpoch
            out.put_i64(-1); // StartOffset (not initialized)
            put_compact_array_len(out, 0); // StateBatches[]
            put_empty_tag_buffer(out);
        }
        put_empty_tag_buffer(out);
    }
...
/// DeleteShareGroupState v0 (key 86). Always flexible.
///
/// Official `DeleteShareGroupStateRequest.json` / `Response.json`.
/// Not KIP-932: parse and reject per-partition **42** `INVALID_REQUEST`
/// (`not KIP-932 share state`). Does not persist share state and does
/// not wrap OffsetCommit / DeleteGroups / InitializeShareGroupState.
/// Official response has no throttle and no top-level error — echo
/// TopicId + Partition with the per-partition code. Unparseable body
/// → empty `Results[]`.
pub(crate) fn encode_delete_share_group_state(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let (group_id, topics) = parse_delete_share_group_state_request(src);
    ...
    write_delete_share_group_state(out, &topics, denied);
}

fn parse_delete_share_group_state_request(
    src: &mut impl Buf,
) -> (String, Vec<([u8; 16], Vec<i32>)>) {
    // Official v0 (flex, `DeleteShareGroupStateRequest.json`):
    // GroupId compact string, Topics[] { TopicId uuid, Partitions[] {
    // Partition i32, tagged }, tagged }, tagged. Echo TopicId +
    // Partition. No StateEpoch / StartOffset (unlike key 83).
    let group_id = get_compact_string(src).unwrap_or_default();
    let mut topics = Vec::new();
    match get_compact_array_len(src) {
        Ok(Some(n)) => {
...
fn write_delete_share_group_state(
    out: &mut BytesMut,
    topics: &[([u8; 16], Vec<i32>)],
    denied: bool,
) {
    // Official DeleteShareGroupStateResponse.json v0:
    // Results[] { TopicId uuid, Partitions[] { Partition i32,
    // ErrorCode i16, ErrorMessage compact nullable string, tagged },
    // tagged }, tagged. No throttleTimeMs. No top-level error.
    put_compact_array_len(out, topics.len());
...
/// ReadShareGroupStateSummary v0 (key 87). Always flexible.
///
/// Official `ReadShareGroupStateSummaryRequest.json` / `Response.json`.
/// Not KIP-932: parse and reject per-partition **42** `INVALID_REQUEST`
/// (`not KIP-932 share state`). Does not persist share state and does
/// not wrap OffsetFetch / OffsetCommit / InitializeShareGroupState /
/// ReadShareGroupState. Official response has no throttle and no
/// top-level error — echo TopicId + Partition with the per-partition
/// code. Unparseable body → empty `Results[]`. Official v1 adds
/// DeliveryCompleteCount (KIP-1226); Volant advertises v0 only and
/// does not write that field.
pub(crate) fn encode_read_share_group_state_summary(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let (group_id, topics) = parse_read_share_group_state_summary_request(src);
    ...
    write_read_share_group_state_summary(out, &topics, denied);
}

fn parse_read_share_group_state_summary_request(
    src: &mut impl Buf,
) -> (String, Vec<([u8; 16], Vec<i32>)>) {
    // Official v0 (flex, `ReadShareGroupStateSummaryRequest.json`):
    // GroupId compact string, Topics[] { TopicId uuid, Partitions[] {
    // Partition i32, LeaderEpoch i32, tagged }, tagged }, tagged.
    // Same shape as ReadShareGroupState 84. Echo TopicId + Partition;
    // discard LeaderEpoch.
    let group_id = get_compact_string(src).unwrap_or_default();
    let mut topics = Vec::new();
    match get_compact_array_len(src) {
...
fn write_read_share_group_state_summary(
    out: &mut BytesMut,
    topics: &[([u8; 16], Vec<i32>)],
    denied: bool,
) {
    // Official ReadShareGroupStateSummaryResponse.json v0:
    // Results[] { TopicId uuid, Partitions[] { Partition i32,
    // ErrorCode i16, ErrorMessage compact nullable string,
    // StateEpoch i32, LeaderEpoch i32, StartOffset i64, tagged },
    // tagged }, tagged. No throttleTimeMs. No top-level error.
    // DeliveryCompleteCount is v1+ and is not written.
    put_compact_array_len(out, topics.len());
    for (topic_id, parts) in topics {
        put_uuid(out, topic_id);
        put_compact_array_len(out, parts.len());
        for partition in parts {
            ...
            out.put_i32(0); // StateEpoch
            out.put_i32(-1); // LeaderEpoch
            out.put_i64(-1); // StartOffset
            put_empty_tag_buffer(out);
        }
        put_empty_tag_buffer(out);
    }
...
/// DescribeShareGroupOffsets v0 (key 90). Always flexible.
///
/// Official `DescribeShareGroupOffsetsRequest.json` / `Response.json`.
/// Official `validVersions` is **0–1** (v1 adds Lag / KIP-1226); Volant
/// advertises **v0 only**. Not KIP-932: parse group ids and reject each
/// group with **42** `INVALID_REQUEST` (`not KIP-932 share offsets`).
/// Does not persist. Does not wrap OffsetFetch 9 / `describe_group` /
/// ConsumerGroupDescribe 69 / ShareGroupDescribe 77. Official response
/// has `throttleTimeMs` and per-group error **after** `Topics[]` (no
/// top-level error besides throttle). Unparseable body → throttle 0 +
/// empty `Groups[]`. Do **not** write Lag (v1+).
pub(crate) fn encode_describe_share_group_offsets(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let ids = parse_describe_share_group_offsets_request(src);

    // Official response: throttleTimeMs, Groups[] { GroupId, Topics[],
    // ErrorCode, ErrorMessage, tagged }, tagged. No top-level error.
    out.put_i32(0); // throttleTimeMs
    put_compact_array_len(out, ids.len());
    for group_id in ids {
        put_compact_string(out, &group_id);
        put_compact_array_len(out, 0); // Topics[] empty — no Lag
        if broker.acls().is_enabled()
            && !broker.acls().authorize(
                Some(principal),
                ResourceType::Group,
...
fn parse_describe_share_group_offsets_request(src: &mut impl Buf) -> Vec<String> {
    // Official v0 (flex, `DescribeShareGroupOffsetsRequest.json`):
    // Groups[] { GroupId compact string, Topics[] nullable {
    // TopicName compact string, Partitions[] i32, tagged }, tagged },
    // tagged. Echo group ids; skip topics/partitions. Do not persist.
    let mut ids = Vec::new();
    match get_compact_array_len(src) {
        Ok(Some(n)) => {
            for _ in 0..n {
                match get_compact_string(src) {
                    Ok(g) => {
                        skip_describe_share_group_offsets_topics(src);
                        let _ = skip_tag_buffer(src);
                        ids.push(g);
                    }
                    Err(_) => break,
...
fn skip_describe_share_group_offsets_topics(src: &mut impl Buf) {
    match get_compact_array_len(src) {
        Ok(Some(n)) => {
            for _ in 0..n {
                if get_compact_string(src).is_err() {
...
/// StreamsGroupTopologyDescriptionUpdate v0 (key 93). Always flexible.
///
/// Official `StreamsGroupTopologyDescriptionUpdateRequest.json` /
/// `Response.json`. Official `validVersions` is **0** only. Not
/// KIP-1071: parse GroupId for ACL, discard MemberId / TopologyEpoch /
/// TopologyDescription, then reject with **42** `INVALID_REQUEST`
/// (`not KIP-1071 streams group`). Does not persist topology. Does
/// not wrap StreamsGroupHeartbeat 88 / StreamsGroupDescribe 89 /
/// classic Heartbeat 12 / ConsumerGroupHeartbeat 68 /
/// ShareGroupHeartbeat 76. Does not join / leave / assign.
/// Official response is throttle + ErrorCode + ErrorMessage + tagged
/// only — no members, no topology echo, no assignment.
pub(crate) fn encode_streams_group_topology_description_update(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    let group_id = parse_streams_group_topology_description_update_request(src);
    ...
    write_streams_group_topology_description_update(out, denied);
}

fn parse_streams_group_topology_description_update_request(src: &mut impl Buf) -> String {
    // Official v0 (flex, `StreamsGroupTopologyDescriptionUpdateRequest.json`):
    // GroupId compact string, MemberId compact string, TopologyEpoch i32,
    // TopologyDescription { Subtopologies[], GlobalStores[], tagged },
    // tagged.
    // Parse GroupId for ACL; discard MemberId / TopologyEpoch / rest.
    // Nested TopologyDescription is not fully decoded. Never panic.
    let group_id = get_compact_string(src).unwrap_or_default();
    let _ = get_compact_string(src);
    if src.remaining() >= 4 {
        let _topology_epoch = src.get_i32();
    }
    group_id
}

fn write_streams_group_topology_description_update(out: &mut BytesMut, denied: bool) {
    // Official StreamsGroupTopologyDescriptionUpdateResponse.json v0:
    // ThrottleTimeMs i32, ErrorCode i16, ErrorMessage compact nullable,
    // tagged. No members / topology echo / assignment.
    out.put_i32(0); // throttleTimeMs
    if denied {
        out.put_i16(KafkaErrorCode::GroupAuthorizationFailed.as_i16());
        put_compact_nullable_string(out, None);
...
mod tests {
    ...
    fn share_heartbeat_v1_body(group: &str, member: &str, epoch: i32) -> BytesMut {
        let mut body = BytesMut::new();
        put_compact_string(&mut body, group);
        put_compact_string(&mut body, member);
        body.put_i32(epoch);
    ...
    #[test]
    fn kafka_share_group_heartbeat_rejects_42() {
        let dir = temp_dir("sgh-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let mut src = share_heartbeat_v1_body("sg-v275", "m1", 1);
        let mut out = BytesMut::new();
        encode_share_group_heartbeat(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, err_msg, member, epoch, interval, assignment) =
            read_heartbeat(&mut resp);
        assert_eq!(throttle, 0);
    ...
    #[test]
    fn kafka_share_group_heartbeat_truncated_still_42() {
        let dir = temp_dir("sgh-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_share_group_heartbeat_acl_deny_is_30() {
        let dir = temp_dir("sgh-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src = share_heartbeat_v1_body("sg-v275", "m1", 0);
        let mut out = BytesMut::new();
        encode_share_group_heartbeat(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, err_msg, member, epoch, interval, assignment) =
    ...
    #[test]
    fn kafka_share_group_heartbeat_does_not_mutate_group() {
        let dir = temp_dir("sgh-keep");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let joined = broker
            ...
            .unwrap();
        assert_eq!(joined.error_code, 0);
        let before = broker.groups().describe_group("sg-v275").unwrap();

        let mut src =
            share_heartbeat_v1_body("sg-v275", &joined.member_id, joined.generation as i32);
        let mut out = BytesMut::new();
        encode_share_group_heartbeat(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (_, error, _, _, _, _, _) = read_heartbeat(&mut resp);
    ...
    fn rsgs_v0_body(group: &str, topic_id: &[u8; 16], partition: i32) -> BytesMut {
        let mut body = BytesMut::new();
        put_compact_string(&mut body, group);
        put_compact_array_len(&mut body, 1);
        put_uuid(&mut body, topic_id);
        put_compact_array_len(&mut body, 1);
        body.put_i32(partition);
        body.put_i32(3); // LeaderEpoch discarded
        put_empty_tag_buffer(&mut body);
        put_empty_tag_buffer(&mut body);
        put_empty_tag_buffer(&mut body);
        body
    ...
    fn read_rsgs(
        src: &mut impl Buf,
    ) -> Vec<([u8; 16], Vec<(i32, i16, Option<String>, i32, i64, usize)>)> {
        let n = get_compact_array_len(src).unwrap().unwrap_or(0);
        let mut results = Vec::new();
        for _ in 0..n {
            let topic_id = get_uuid(src).unwrap();
            ...
            for _ in 0..pn {
                ...
                let error = src.get_i16();
                let err_msg = get_compact_nullable_string(src).unwrap();
                let state_epoch = src.get_i32();
                let start_offset = src.get_i64();
                let batches = get_compact_array_len(src).unwrap().unwrap_or(0);
                skip_tag_buffer(src).unwrap();
                parts.push((
                    partition,
                    error,
    ...
    #[test]
    fn kafka_read_share_group_state_rejects_42() {
        let dir = temp_dir("rsgs-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let topic_id = [0x44u8; 16];
        let mut src = rsgs_v0_body("sg-v280", &topic_id, 7);
        let mut out = BytesMut::new();
        encode_read_share_group_state(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_rsgs(&mut resp);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].0, topic_id);
        assert_eq!(results[0].1.len(), 1);
        assert_eq!(results[0].1[0].0, 7);
        ...
        assert_eq!(results[0].1[0].3, 0);
        assert_eq!(results[0].1[0].4, -1);
        assert_eq!(results[0].1[0].5, 0);
        assert!(broker.groups().describe_group("sg-v280").is_none());
        assert!(broker
            .groups()
            .fetch_offsets("sg-v280", &[])
            .unwrap()
            .entries
            .is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    ...
    #[test]
    fn kafka_read_share_group_state_truncated_is_empty_results() {
        let dir = temp_dir("rsgs-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_read_share_group_state_acl_deny_is_30() {
        let dir = temp_dir("rsgs-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src = rsgs_v0_body("sg-v280", &topic_id, 1);
        let mut out = BytesMut::new();
        encode_read_share_group_state(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_rsgs(&mut resp);
    ...
    fn dsgs_v0_body(group: &str, topic_id: &[u8; 16], partition: i32) -> BytesMut {
        let mut body = BytesMut::new();
        put_compact_string(&mut body, group);
        put_compact_array_len(&mut body, 1);
        put_uuid(&mut body, topic_id);
    ...
    #[test]
    fn kafka_delete_share_group_state_rejects_42() {
        let dir = temp_dir("dsgs-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let topic_id = [0x44u8; 16];
        let mut src = dsgs_v0_body("sg-v282", &topic_id, 7);
        let mut out = BytesMut::new();
        encode_delete_share_group_state(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_isgs(&mut resp);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].0, topic_id);
        ...
        assert!(broker.groups().describe_group("sg-v282").is_none());
        assert!(broker
            .groups()
            .fetch_offsets("sg-v282", &[])
            .unwrap()
            .entries
            .is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    ...
    #[test]
    fn kafka_delete_share_group_state_truncated_is_empty_results() {
        let dir = temp_dir("dsgs-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_delete_share_group_state_acl_deny_is_30() {
        let dir = temp_dir("dsgs-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let topic_id = [0x55u8; 16];
        let mut src = dsgs_v0_body("sg-v282", &topic_id, 1);
        let mut out = BytesMut::new();
        encode_delete_share_group_state(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_isgs(&mut resp);
    ...
    fn rsgs_summary_v0_body(group: &str, topic_id: &[u8; 16], partition: i32) -> BytesMut {
        let mut body = BytesMut::new();
        put_compact_string(&mut body, group);
        put_compact_array_len(&mut body, 1);
        put_uuid(&mut body, topic_id);
    ...
    fn read_rsgs_summary(
        src: &mut impl Buf,
    ) -> Vec<([u8; 16], Vec<(i32, i16, Option<String>, i32, i32, i64)>)> {
        let n = get_compact_array_len(src).unwrap().unwrap_or(0);
        let mut results = Vec::new();
        for _ in 0..n {
            let topic_id = get_uuid(src).unwrap();
    ...
    #[test]
    fn kafka_read_share_group_state_summary_rejects_42() {
        let dir = temp_dir("rsgs-sum-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let topic_id = [0x44u8; 16];
        let mut src = rsgs_summary_v0_body("sg-v283", &topic_id, 7);
        let mut out = BytesMut::new();
        encode_read_share_group_state_summary(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_rsgs_summary(&mut resp);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].0, topic_id);
        assert_eq!(results[0].1.len(), 1);
        assert_eq!(results[0].1[0].0, 7);
        ...
        assert_eq!(results[0].1[0].3, 0);
        assert_eq!(results[0].1[0].4, -1);
        assert_eq!(results[0].1[0].5, -1);
        assert!(broker.groups().describe_group("sg-v283").is_none());
        assert!(broker
            .groups()
            .fetch_offsets("sg-v283", &[])
            .unwrap()
            .entries
            .is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    ...
    #[test]
    fn kafka_read_share_group_state_summary_truncated_is_empty_results() {
        let dir = temp_dir("rsgs-sum-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_read_share_group_state_summary_acl_deny_is_30() {
        let dir = temp_dir("rsgs-sum-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let topic_id = [0x55u8; 16];
        let mut src = rsgs_summary_v0_body("sg-v283", &topic_id, 1);
        let mut out = BytesMut::new();
        encode_read_share_group_state_summary(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let results = read_rsgs_summary(&mut resp);
    ...
    fn read_dsgo(src: &mut impl Buf) -> (i32, Vec<(String, usize, i16, Option<String>)>) {
        let throttle = src.get_i32();
        let n = get_compact_array_len(src).unwrap().unwrap_or(0);
        let mut groups = Vec::new();
        for _ in 0..n {
            let group_id = get_compact_string(src).unwrap();
            let n_topics = get_compact_array_len(src).unwrap().unwrap_or(0);
            let error = src.get_i16();
            let err_msg = get_compact_nullable_string(src).unwrap();
            skip_tag_buffer(src).unwrap();
            groups.push((group_id, n_topics, error, err_msg));
        }
        skip_tag_buffer(src).unwrap();
        assert_eq!(src.remaining(), 0);
        (throttle, groups)
    ...
    #[test]
    fn kafka_describe_share_group_offsets_rejects_42() {
        let dir = temp_dir("dsgo-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let mut src = dsgo_v0_body("sg-v284", "events", 0);
        let mut out = BytesMut::new();
        encode_describe_share_group_offsets(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, groups) = read_dsgo(&mut resp);
        assert_eq!(throttle, 0);
        assert_eq!(groups.len(), 1);
        assert_eq!(groups[0].0, "sg-v284");
        assert_eq!(groups[0].1, 0); // empty Topics[]
        assert_eq!(groups[0].2, KafkaErrorCode::InvalidRequest.as_i16());
        assert_eq!(groups[0].3.as_deref(), Some("not KIP-932 share offsets"));
        assert!(broker.groups().describe_group("sg-v284").is_none());
        assert!(broker
            .groups()
            .fetch_offsets("sg-v284", &[])
            .unwrap()
            .entries
            .is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    ...
    #[test]
    fn kafka_describe_share_group_offsets_does_not_wrap_classic() {
        let dir = temp_dir("dsgo-wrap");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let joined = broker
            ...
            .unwrap();
        assert_eq!(joined.error_code, 0);
        assert!(broker.groups().describe_group("sg-v284").is_some());
        let before = broker.groups().fetch_offsets("sg-v284", &[]).unwrap();

        let mut src = dsgo_v0_body("sg-v284", "events", 0);
        let mut out = BytesMut::new();
        encode_describe_share_group_offsets(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (_, groups) = read_dsgo(&mut resp);
        assert_eq!(groups[0].2, KafkaErrorCode::InvalidRequest.as_i16());
        assert_eq!(groups[0].3.as_deref(), Some("not KIP-932 share offsets"));
        assert_eq!(
            broker
                .groups()
                .describe_group("sg-v284")
                .unwrap()
                .members
                .len(),
            1
        );
        let after = broker.groups().fetch_offsets("sg-v284", &[]).unwrap();
        assert_eq!(after.entries.len(), before.entries.len());
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn kafka_describe_share_group_offsets_truncated_is_empty_groups() {
        let dir = temp_dir("dsgo-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_describe_share_group_offsets_acl_deny_is_30() {
        let dir = temp_dir("dsgo-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    fn streams_topology_update_v0_body(group: &str, member: &str, epoch: i32) -> BytesMut {
        let mut body = BytesMut::new();
        put_compact_string(&mut body, group);
        put_compact_string(&mut body, member);
        body.put_i32(epoch);
    ...
    fn read_streams_topology_update(src: &mut impl Buf) -> (i32, i16, Option<String>) {
        let throttle = src.get_i32();
        let error = src.get_i16();
        let err_msg = get_compact_nullable_string(src).unwrap();
        skip_tag_buffer(src).unwrap();
        assert_eq!(src.remaining(), 0);
        (throttle, error, err_msg)
    }

    #[test]
    fn kafka_streams_group_topology_description_update_rejects_42() {
        let dir = temp_dir("sgtdu-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let mut src = streams_topology_update_v0_body("stg-v291", "m1", 1);
        let mut out = BytesMut::new();
        encode_streams_group_topology_description_update(
            &broker,
            &mut src,
            &mut out,
            "kafka-anonymous",
        ...
        let mut resp = out.freeze();
        let (throttle, error, err_msg) = read_streams_topology_update(&mut resp);
        assert_eq!(throttle, 0);
        assert_eq!(error, KafkaErrorCode::InvalidRequest.as_i16());
        assert_eq!(err_msg.as_deref(), Some("not KIP-1071 streams group"));
    ...
    #[test]
    fn kafka_streams_group_topology_description_update_truncated_still_42() {
        let dir = temp_dir("sgtdu-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_streams_group_topology_description_update_acl_deny_is_30() {
        let dir = temp_dir("sgtdu-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src = streams_topology_update_v0_body("stg-v291", "m1", 0);
        let mut out = BytesMut::new();
        encode_streams_group_topology_description_update(
            &broker,
            &mut src,
    ...
    #[test]
    fn kafka_streams_group_topology_description_update_does_not_mutate_group() {
        let dir = temp_dir("sgtdu-keep");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        let joined = broker
            ...
            .unwrap();
        assert_eq!(joined.error_code, 0);
        let before = broker.groups().describe_group("stg-v291").unwrap();

        let mut src = streams_topology_update_v0_body(
            "stg-v291",
            &joined.member_id,
            joined.generation as i32,
        );
        let mut out = BytesMut::new();
        encode_streams_group_topology_description_update(
            &broker,
        ...
        let mut resp = out.freeze();
        let (_, error, _) = read_streams_topology_update(&mut resp);
        assert_eq!(error, KafkaErrorCode::InvalidRequest.as_i16());

        let after = broker.groups().describe_group("stg-v291").unwrap();
        assert_eq!(after.generation, before.generation);
        assert_eq!(after.members.len(), before.members.len());
        assert_eq!(after.members[0].member_id, before.members[0].member_id);
        let _ = std::fs::remove_dir_all(&dir);
