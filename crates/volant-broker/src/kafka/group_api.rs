...
fn parse_share_fetch_request(src: &mut impl Buf) -> String {
    ...
    // ShareAcquireMode / IsRenewAck are v2+ — out of advertised range.
                            ...
                            match get_compact_array_len(src) {
                                Ok(Some(bn)) => {
                                    for _ in 0..bn {
                                        ...
                                        let _ = src.get_i64(); // FirstOffset
                                        let _ = src.get_i64(); // LastOffset
                                        match get_compact_array_len(src) {
                                            Ok(Some(tn)) => {
                                                for _ in 0..tn {
                                                    if src.remaining() < 1 {
...
mod tests {
    ...
    fn share_fetch_v1_body(group: Option<&str>) -> BytesMut {
        ...
        put_empty_tag_buffer(&mut body); // AcknowledgementBatch tags
        ...
        put_compact_array_len(&mut body, 1);
        body.put_i32(0);
        put_empty_tag_buffer(&mut body); // ForgottenTopic tags
