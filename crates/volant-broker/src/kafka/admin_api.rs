...
/// BeginQuorumEpoch v1 (always flexible). Volant is **not** a KRaft
/// quorum and does **not** persist voter/epoch state.
///
/// Parses official Kafka `BeginQuorumEpochRequest.json` v1 fields
/// (`clusterId`, `voterId`, `topics[]` with `voterDirectoryId` /
/// leader id/epoch, `leaderEndpoints[]`) and discards them. Does
/// **not** persist. Does **not** wrap openraft RequestVote or Vote
/// 52. Returns error **42** `INVALID_REQUEST`
/// (`not KRaft quorum epoch`), empty `topics[]`. Official
/// `BeginQuorumEpochResponse.json` v1 has **no** `throttleTimeMs`.
/// Controller is not required. ACL: Cluster **ALTER** (disabled ACLs
/// allow). Denied → **31**, empty topics.
pub(crate) fn encode_begin_quorum_epoch(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    parse_begin_quorum_epoch_request(src);
    ...
    write_begin_quorum_epoch(out, error);
}

fn parse_begin_quorum_epoch_request(src: &mut impl Buf) {
    // Official v1 (flex, `BeginQuorumEpochRequest.json`):
    // ClusterId compact nullable string,
    // VoterId i32 (v1+),
    // Topics[] compact {
    //   TopicName compact string,
    //   Partitions[] {
    //     PartitionIndex i32, VoterDirectoryId uuid (v1+),
    //     LeaderId i32, LeaderEpoch i32, tagged
    //   },
    //   tagged
    // },
        ...
        Ok(Some(n)) => {
            for _ in 0..n {
                ...
                match get_compact_array_len(src) {
                    Ok(Some(pn)) => {
                        for _ in 0..pn {
                            // PartitionIndex + VoterDirectoryId + LeaderId
                            // + LeaderEpoch
                            if src.remaining() < 4 + 16 + 4 + 4 {
                                break;
                            }
                            let _partition = src.get_i32();
                            let _ = get_uuid(src);
...
/// AddRaftVoter v0 (always flexible). Volant is **not** a KRaft
/// controller (no voter set / DirectoryId store). Does **not** wrap
/// native `add_broker`. Overlay membership is unchanged.
///
/// Parses official Kafka `AddRaftVoterRequest.json` v0 fields
/// (`clusterId`, `timeoutMs`, `voterId`, `voterDirectoryId`,
/// `listeners[]`) and discards them. AckWhenCommitted is v1+ and is
/// not parsed. Returns throttle **0**, error **42** `INVALID_REQUEST`,
/// errorMessage `"not KRaft raft voter"`. Controller is not required.
/// ACL: Cluster **ALTER** (disabled ACLs allow). Denied → **31**,
/// errorMessage null.
pub(crate) fn encode_add_raft_voter(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    parse_add_raft_voter_request(src);
    ...
    let (error, msg) = if denied {
        ...
        (KafkaErrorCode::InvalidRequest, Some("not KRaft raft voter"))
    };

    write_add_raft_voter(out, error, msg);
}

fn parse_add_raft_voter_request(src: &mut impl Buf) {
    // Official v0 (flex, `AddRaftVoterRequest.json`):
    // ClusterId compact nullable string, TimeoutMs i32, VoterId i32,
    // VoterDirectoryId uuid,
    // Listeners[] { name, host, port u16, tagged }, tagged.
...
fn write_add_raft_voter(out: &mut BytesMut, error: KafkaErrorCode, msg: Option<&str>) {
    // Official AddRaftVoterResponse.json (v0):
    // throttleTimeMs, errorCode, errorMessage compact nullable, tagged.
    out.put_i32(0); // throttleTimeMs
    out.put_i16(error.as_i16());
    put_compact_nullable_string(out, msg);
...
/// RemoveRaftVoter v0 (always flexible). Volant membership is overlay
/// `membership.json` + native `remove_broker` — **not** KRaft voter
/// membership.
///
/// Parses official Kafka `RemoveRaftVoterRequest.json` v0 fields
/// (`clusterId`, `voterId`, `voterDirectoryId`) and discards them.
/// Does **not** persist. Does **not** call `remove_broker`. Returns
/// throttle **0**, error **42** `INVALID_REQUEST`, errorMessage
/// `"not KRaft raft voter"`. Controller is not required. ACL: Cluster
/// **ALTER** (disabled ACLs allow). Denied → **31**, errorMessage null.
pub(crate) fn encode_remove_raft_voter(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    parse_remove_raft_voter_request(src);
    ...
    write_remove_raft_voter(out, error, msg);
}

fn parse_remove_raft_voter_request(src: &mut impl Buf) {
    // Official v0 (flex, `RemoveRaftVoterRequest.json`):
    // ClusterId compact nullable string, VoterId i32, VoterDirectoryId uuid,
    // tagged.
    let _ = get_compact_nullable_string(src);
    if src.remaining() >= 4 {
        let _voter_id = src.get_i32();
...
fn write_remove_raft_voter(out: &mut BytesMut, error: KafkaErrorCode, msg: Option<&str>) {
    // Official RemoveRaftVoterResponse.json (v0):
    // throttleTimeMs, errorCode, errorMessage compact nullable, tagged.
    out.put_i32(0); // throttleTimeMs
    out.put_i16(error.as_i16());
    put_compact_nullable_string(out, msg);
...
/// UnregisterController v0 (always flexible). Volant is **not** a
/// KRaft controller quorum.
///
/// Parses official Kafka `UnregisterControllerRequest.json` v0
/// (`controllerId` i32 + tagged) and discards it. Does **not** persist.
/// Does **not** call `remove_broker`. Returns throttle **0**, error
/// **42** `INVALID_REQUEST`, errorMessage
/// `"not KRaft controller unregister"`. Controller is not required.
/// ACL: Cluster **ALTER** (disabled ACLs allow). Denied → **31**,
/// errorMessage null.
pub(crate) fn encode_unregister_controller(
    broker: &Broker,
    src: &mut impl Buf,
    out: &mut BytesMut,
    principal: &str,
) {
    parse_unregister_controller_request(src);
    ...
    write_unregister_controller(out, error, msg);
}

fn parse_unregister_controller_request(src: &mut impl Buf) {
    // Official v0 (flex, `UnregisterControllerRequest.json`):
    // ControllerId i32, tagged.
    if src.remaining() >= 4 {
        let _controller_id = src.get_i32();
    }
    let _ = skip_tag_buffer(src);
...
fn write_unregister_controller(out: &mut BytesMut, error: KafkaErrorCode, msg: Option<&str>) {
    // Official UnregisterControllerResponse.json (v0):
    // throttleTimeMs, errorCode, errorMessage compact nullable, tagged.
    out.put_i32(0); // throttleTimeMs
    out.put_i16(error.as_i16());
    put_compact_nullable_string(out, msg);
...
mod tests {
    ...
    fn vote_body(
        ...
        for (name, partitions) in topics {
            put_compact_string(&mut body, name);
            put_compact_array_len(&mut body, partitions.len());
            for (partition, replica_epoch, replica_id, last_offset_epoch, last_offset) in
                *partitions
            {
                body.put_i32(*partition);
    ...
    fn read_begin_quorum_epoch(src: &mut impl Buf) -> i16 {
        let error = src.get_i16();
        assert_eq!(get_compact_array_len(src).unwrap(), Some(0));
        skip_tag_buffer(src).unwrap();
        assert_eq!(src.remaining(), 0);
    ...
    #[test]
    fn kafka_begin_quorum_epoch_rejects_and_does_not_persist() {
        let dir = temp_dir("bqe-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        directory[15] = 0x53;
        let mut src = begin_quorum_epoch_body(
            Some("volant-cluster"),
            2,
        ...
        let mut out = BytesMut::new();
        encode_begin_quorum_epoch(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        assert_eq!(
    ...
    #[test]
    fn kafka_begin_quorum_epoch_truncated_still_42() {
        let dir = temp_dir("bqe-trunc");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
    ...
    #[test]
    fn kafka_begin_quorum_epoch_acl_deny_is_31() {
        let dir = temp_dir("bqe-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src =
            begin_quorum_epoch_body(Some("c"), 2, &[("events", &[(0, [0u8; 16], 1, 1)])], &[]);
        let mut out = BytesMut::new();
        encode_begin_quorum_epoch(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        assert_eq!(
    ...
    #[test]
    fn kafka_begin_quorum_epoch_not_controller_still_42() {
        let dir = temp_dir("bqe-nc");
        let broker = cluster_n2(dir.clone(), 2);
        assert!(!broker.is_controller());
        let before_ids = overlay_ids(&broker);
        let before_raft = raft_state(&broker);
        ...
        let mut src =
            begin_quorum_epoch_body(None, -1, &[("events", &[(0, [0u8; 16], 3, 0)])], &[]);
        let mut out = BytesMut::new();
        encode_begin_quorum_epoch(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        assert_eq!(
    ...
    fn read_add_raft_voter(src: &mut impl Buf) -> (i32, i16, Option<String>) {
        let throttle = src.get_i32();
        let error = src.get_i16();
        let msg = get_compact_nullable_string(src).unwrap();
        skip_tag_buffer(src).unwrap();
    ...
    #[test]
    fn kafka_add_raft_voter_rejects_and_does_not_persist() {
        let dir = temp_dir("arv-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut directory = [0u8; 16];
        directory[15] = 0x80;
        let mut src = add_raft_voter_body(
            Some("volant-cluster"),
            5_000,
            4,
            &directory,
        ...
        let mut out = BytesMut::new();
        encode_add_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_add_raft_voter(&mut resp);
        assert_eq!(throttle, 0);
        assert_eq!(error, KafkaErrorCode::InvalidRequest.as_i16());
    ...
    #[test]
    fn kafka_add_raft_voter_existing_brokers_unchanged() {
        let dir = temp_dir("arv-exist");
        let broker = cluster_n2(dir.clone(), 1);
        broker
            .add_broker(3, "127.0.0.1".into(), 19403, None)
            .unwrap();
    ...
    #[test]
    fn kafka_add_raft_voter_acl_deny_is_31() {
        let dir = temp_dir("arv-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut directory = [0u8; 16];
        directory[1] = 0xbb;
        let mut src = add_raft_voter_body(Some("c"), 0, 2, &directory, &[]);
        let mut out = BytesMut::new();
        encode_add_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_add_raft_voter(&mut resp);
    ...
    #[test]
    fn kafka_add_raft_voter_not_controller_still_42() {
        let dir = temp_dir("arv-nc");
        let broker = cluster_n2(dir.clone(), 2);
        assert!(!broker.is_controller());
        let before = overlay_ids(&broker);
    ...
    fn read_remove_raft_voter(src: &mut impl Buf) -> (i32, i16, Option<String>) {
        let throttle = src.get_i32();
        let error = src.get_i16();
        let msg = get_compact_nullable_string(src).unwrap();
        skip_tag_buffer(src).unwrap();
    ...
    #[test]
    fn kafka_remove_raft_voter_rejects_and_does_not_persist() {
        let dir = temp_dir("rmvoter-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut directory = [0u8; 16];
        directory[15] = 0x81;
        let mut src = remove_raft_voter_body(Some("lkc-test"), 1, &directory);
        let mut out = BytesMut::new();
        encode_remove_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_remove_raft_voter(&mut resp);
        assert_eq!(throttle, 0);
        assert_eq!(error, KafkaErrorCode::InvalidRequest.as_i16());
        assert_eq!(msg.as_deref(), Some("not KRaft raft voter"));
    ...
    #[test]
    fn kafka_remove_raft_voter_existing_brokers_unchanged() {
        let dir = temp_dir("rmvoter-exist");
        let broker = cluster_n2(dir.clone(), 1);
        broker
            .add_broker(3, "127.0.0.1".into(), 19403, None)
            .unwrap();
        ...
        let mut directory = [0u8; 16];
        directory[0] = 0xaa;
        let mut src = remove_raft_voter_body(None, 3, &directory);
        let mut out = BytesMut::new();
        encode_remove_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_remove_raft_voter(&mut resp);
    ...
    #[test]
    fn kafka_remove_raft_voter_acl_deny_is_31() {
        let dir = temp_dir("rmvoter-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut directory = [0u8; 16];
        directory[1] = 0xbb;
        let mut src = remove_raft_voter_body(Some("cluster"), 2, &directory);
        let mut out = BytesMut::new();
        encode_remove_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_remove_raft_voter(&mut resp);
    ...
    #[test]
    fn kafka_remove_raft_voter_not_controller_still_42() {
        let dir = temp_dir("rmvoter-nc");
        let broker = cluster_n2(dir.clone(), 2);
        assert!(!broker.is_controller());
        let before = overlay_ids(&broker);
        ...
        let mut directory = [0u8; 16];
        directory[2] = 0xcc;
        let mut src = remove_raft_voter_body(None, 1, &directory);
        let mut out = BytesMut::new();
        encode_remove_raft_voter(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_remove_raft_voter(&mut resp);
    ...
    fn unregister_controller_body(controller_id: i32) -> BytesMut {
        let mut body = BytesMut::new();
        body.put_i32(controller_id);
        put_empty_tag_buffer(&mut body);
        body
    ...
    fn read_unregister_controller(src: &mut impl Buf) -> (i32, i16, Option<String>) {
        let throttle = src.get_i32();
        let error = src.get_i16();
        let msg = get_compact_nullable_string(src).unwrap();
        skip_tag_buffer(src).unwrap();
    ...
    #[test]
    fn kafka_unregister_controller_rejects_and_does_not_persist() {
        let dir = temp_dir("unregc-ok");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src = unregister_controller_body(1);
        let mut out = BytesMut::new();
        encode_unregister_controller(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_unregister_controller(&mut resp);
        assert_eq!(throttle, 0);
        assert_eq!(error, KafkaErrorCode::InvalidRequest.as_i16());
        assert_eq!(msg.as_deref(), Some("not KRaft controller unregister"));
    ...
    #[test]
    fn kafka_unregister_controller_existing_brokers_unchanged() {
        let dir = temp_dir("unregc-exist");
        let broker = cluster_n2(dir.clone(), 1);
        broker
            .add_broker(3, "127.0.0.1".into(), 19403, None)
            .unwrap();
        ...
        let mut src = unregister_controller_body(3);
        let mut out = BytesMut::new();
        encode_unregister_controller(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_unregister_controller(&mut resp);
    ...
    #[test]
    fn kafka_unregister_controller_acl_deny_is_31() {
        let dir = temp_dir("unregc-acl");
        let broker = Broker::new(StorageConfig {
            data_dir: dir.clone(),
            ..StorageConfig::default()
        });
        ...
        let mut src = unregister_controller_body(2);
        let mut out = BytesMut::new();
        encode_unregister_controller(&broker, &mut src, &mut out, "kafka-anonymous");
        let mut resp = out.freeze();
        let (throttle, error, msg) = read_unregister_controller(&mut resp);
    ...
    #[test]
    fn kafka_unregister_controller_not_controller_still_42() {
        let dir = temp_dir("unregc-nc");
        let broker = cluster_n2(dir.clone(), 2);
        assert!(!broker.is_controller());
        let before = overlay_ids(&broker);
