...
async fn dispatch_kafka(
    ...
    let flexible_response_header = matches!(
        ...
        (
            Some(ApiKey::DescribeCluster)
                ...
                | Some(ApiKey::ReadShareGroupState)
                | Some(ApiKey::WriteShareGroupState)
                ...
                | Some(ApiKey::AlterShareGroupOffsets)
                | Some(ApiKey::DeleteShareGroupOffsets)
    ...
    match api {
        ...
        Some(ApiKey::StreamsGroupHeartbeat) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "streams group heartbeat flexible header tag buffer");
            }
            group_api::encode_streams_group_heartbeat(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::ShareFetch) if hdr.api_version == 1 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "share fetch flexible header tag buffer");
            }
            group_api::encode_share_fetch(broker, &mut src, &mut out, principal);
        }
        ...
        Some(ApiKey::ShareGroupDescribe) if hdr.api_version == 1 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
        ...
        Some(ApiKey::AddRaftVoter) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "add raft voter flexible header tag buffer");
            }
            admin_api::encode_add_raft_voter(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::RemoveRaftVoter) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
        ...
        Some(ApiKey::InitializeShareGroupState) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
        ...
        Some(ApiKey::ReadShareGroupState) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "read share group state flexible header tag buffer");
            }
            group_api::encode_read_share_group_state(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::WriteShareGroupState) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "write share group state flexible header tag buffer");
            }
            group_api::encode_write_share_group_state(broker, &mut src, &mut out, principal);
        }
        ...
        Some(ApiKey::ReadShareGroupStateSummary) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(
                    error = %e,
        ...
        Some(ApiKey::DescribeShareGroupOffsets) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "describe share group offsets flexible header tag buffer");
            }
            group_api::encode_describe_share_group_offsets(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::AlterShareGroupOffsets) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "alter share group offsets flexible header tag buffer");
            }
            group_api::encode_alter_share_group_offsets(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::DeleteShareGroupOffsets) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(error = %e, "delete share group offsets flexible header tag buffer");
            }
            group_api::encode_delete_share_group_offsets(broker, &mut src, &mut out, principal);
        }
        Some(ApiKey::StreamsGroupTopologyDescriptionUpdate) if hdr.api_version == 0 => {
            if let Err(e) = skip_tag_buffer(&mut src) {
                debug!(
                    error = %e,
                    "streams group topology description update flexible header tag buffer"
                );
            }
            group_api::encode_streams_group_topology_description_update(
