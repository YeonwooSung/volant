//! Opt-in openraft metadata leader election (v0.11) + InstallSnapshot (v0.17).
//!
//! Election + snapshot/log truncation on the **openraft** path only.
//! Homemade Phase 154 `metadata_raft.rs` is unchanged (no InstallSnapshot).
//! Assignment apply stays Noop / `assignment.json` (bytes may be included in
//! the snapshot payload; install does not rewrite live assignment).
