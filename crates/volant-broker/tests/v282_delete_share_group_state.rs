...
#[tokio::test]
async fn api_versions_lists_delete_share_group_state_86() {
    let (_dir, broker) = broker_temp("v282", "api");
    ...
    assert!(found.len() >= 80);
    assert_eq!(found.get(&86), Some(&(0, 0)));
...
#[tokio::test]
async fn delete_share_group_state_v0_is_42_nothing_persisted() {
    let (_dir, broker) = broker_temp("v282", "reject");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    ...
    let resp = rpc(
        &addr,
        encode_request_flexible(86, 0, 10, Some("c"), &dsgs_v0_body("sg-v282", &topic_id, 0)),
    )
    .await;
    let mut src = resp.freeze();
    ...
    let offsets = broker.groups().fetch_offsets("sg-v282", &[]).unwrap();
    assert!(
        offsets.entries.is_empty(),
        "DeleteShareGroupState must not persist offsets/share state"
    );
...
#[tokio::test]
async fn delete_share_group_state_v1_is_35() {
    let (_dir, broker) = broker_temp("v282", "v1");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;

    let resp = rpc(&addr, encode_request_flexible(86, 1, 99, Some("c"), &[])).await;
    let mut src = resp.freeze();
...
#[tokio::test]
async fn delete_share_group_state_acl_deny_is_30() {
    let (_dir, broker) = broker_temp("v282", "acl");
    broker
    ...
    let resp = rpc(
        ...
        encode_request_flexible(
            ...
            11,
            Some("c"),
            &dsgs_v0_body("sg-v282-acl", &topic_id, 3),
        ),
    )
    .await;
    let mut src = resp.freeze();
    ...
    assert!(broker.groups().describe_group("sg-v282-acl").is_none());
    assert!(broker
        .groups()
        .fetch_offsets("sg-v282-acl", &[])
        .unwrap()
        .entries
        .is_empty());
