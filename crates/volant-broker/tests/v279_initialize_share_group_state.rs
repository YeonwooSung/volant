...
#[tokio::test]
async fn api_versions_lists_initialize_share_group_state_83() {
    let (_dir, broker) = broker_temp("v279", "api");
    ...
    assert!(found.len() >= 75);
    assert_eq!(found.get(&83), Some(&(0, 0)));
    assert_eq!(found.get(&82), Some(&(0, 0))); // UpdateRaftVoter still listed
...
#[tokio::test]
async fn initialize_share_group_state_v0_is_42_nothing_persisted() {
    let (_dir, broker) = broker_temp("v279", "reject");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    ...
    let resp = rpc(
        ...
        encode_request_flexible(
            ...
            10,
            Some("c"),
            &isgs_v0_body("sg-v279", &topic_id, 0),
        ),
    )
    .await;
    let mut src = resp.freeze();
    ...
    let offsets = broker.groups().fetch_offsets("sg-v279", &[]).unwrap();
    assert!(
        offsets.entries.is_empty(),
        "InitializeShareGroupState must not persist offsets/share state"
    );
...
#[tokio::test]
async fn initialize_share_group_state_v1_is_35() {
    let (_dir, broker) = broker_temp("v279", "v1");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;

    let resp = rpc(&addr, encode_request_flexible(83, 1, 99, Some("c"), &[])).await;
    let mut src = resp.freeze();
