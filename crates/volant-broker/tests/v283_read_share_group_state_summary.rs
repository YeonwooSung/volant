...
#[tokio::test]
async fn api_versions_lists_read_share_group_state_summary_87() {
    let (_dir, broker) = broker_temp("v283", "api");
    ...
    assert!(found.len() >= 80);
    assert_eq!(found.get(&87), Some(&(0, 0)));
...
#[tokio::test]
async fn read_share_group_state_summary_v0_is_42_nothing_persisted() {
    let (_dir, broker) = broker_temp("v283", "reject");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    ...
    let resp = rpc(
        ...
        encode_request_flexible(
            ...
            10,
            Some("c"),
            &rsgs_summary_v0_body("sg-v283", &topic_id, 0),
        ),
    )
    .await;
    let mut src = resp.freeze();
    ...
    assert_eq!(src.get_i32(), -1); // LeaderEpoch
    ...
    let offsets = broker.groups().fetch_offsets("sg-v283", &[]).unwrap();
    assert!(
        offsets.entries.is_empty(),
        "ReadShareGroupStateSummary must not persist offsets/share state"
    );
...
#[tokio::test]
async fn read_share_group_state_summary_v1_is_35() {
    let (_dir, broker) = broker_temp("v283", "v1");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;

    let resp = rpc(&addr, encode_request_flexible(87, 1, 99, Some("c"), &[])).await;
    let mut src = resp.freeze();
...
#[tokio::test]
async fn read_share_group_state_summary_acl_deny_is_30() {
    let (_dir, broker) = broker_temp("v283", "acl");
    broker
    ...
    let resp = rpc(
        ...
        encode_request_flexible(
            ...
            11,
            Some("c"),
            &rsgs_summary_v0_body("sg-v283", &topic_id, 2),
        ),
    )
    .await;
    let mut src = resp.freeze();
