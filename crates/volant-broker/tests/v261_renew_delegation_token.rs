...
fn renew_token_v0(hmac: &[u8], renew_period_ms: i64) -> BytesMut {
...
#[tokio::test]
async fn api_versions_lists_renew_delegation_token_39() {
    let (_dir, broker) = broker_temp("v261", "api");
    ...
    assert!(found.len() >= 61);
    assert_eq!(found.get(&39), Some(&(0, 0)));
...
#[tokio::test]
async fn renew_delegation_token_v0_is_42_nothing_persisted() {
    let (dir, broker) = broker_temp("v261", "renew");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    ...
    assert!(
        ...
        "RenewDelegationToken must not persist a token"
    );
...
#[tokio::test]
async fn renew_delegation_token_v1_unsupported() {
    let (_dir, broker) = broker_temp("v261", "v1");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    let resp = rpc(&addr, encode_request_flexible(39, 1, 99, Some("c"), &[])).await;
    let mut src = resp.freeze();
