...
impl ClusterHarness {
    ...
    /// Abort the native protocol listener for `id` (1..=3) so peer RPCs fail.
...
/// Create on owner → peers receive mirror via fan-out.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn create_mirrors_to_peers() {
    let h = ClusterHarness::boot().await;
    h.b1.create_topic("mirr", 1).unwrap();
    propagate(&[&h.b1, &h.b2, &h.b3], "mirr").await;
    ...
    produce_one(leader_kafka, "mirr", b"a").await;
    catch_up_isr(leader, "mirr");

    let body = fetch_v12("mirr", 1, 0, 0);
    let resp = kafka_rpc(
        leader_kafka,
    ...
    // Ensure owner drains pending puts (schedule may race; force fan-out).
...
/// While owner is alive, peer still forwards (primary empty on peer).
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn happy_path_still_forwards_while_owner_up() {
    let h = ClusterHarness::boot().await;
    h.b1.create_topic("live", 1).unwrap();
    propagate(&[&h.b1, &h.b2, &h.b3], "live").await;
    ...
    produce_one(leader_kafka, "live", b"a").await;
    catch_up_isr(leader, "live");

    let body = fetch_v12("live", 1, 0, 0);
    let resp = kafka_rpc(
        leader_kafka,
        encode_request_flexible(1, 12, 10, Some("c"), &body),
    )
    ...
    assert!(
        ...
        "must still forward while owner up"
    );
    assert_eq!(
        peer.fetch_sessions().promote_total(),
        promote_before,
        "must not promote while owner reachable"
    );
    assert_eq!(peer.fetch_sessions().active_count(), 0);
}
...
async fn promote_on_owner_unreachable() {
    ...
    // Kill owner native listener so peer KafkaFetchForward RPC fails.
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn no_mirror_owner_dead_is_70() {
    let mut h = ClusterHarness::boot().await;
    h.b1.create_topic("gone", 1).unwrap();
    propagate(&[&h.b1, &h.b2, &h.b3], "gone").await;
    ...
    produce_one(&leader_kafka, "gone", b"a").await;
    catch_up_isr(&leader, "gone");

    let body = fetch_v12("gone", 1, 0, 0);
    let resp = kafka_rpc(
        &leader_kafka,
        encode_request_flexible(1, 12, 10, Some("c"), &body),
    )
    ...
    // Do NOT fan-out; drop owner pending + any peer mirror that raced in.
    ...
    assert!(
        !peer.fetch_sessions().mirror_contains(sid),
        "test requires no mirror on peer"
    );
    ...
    let mut src = resp.freeze();
    let (top_err, echo) = assert_flex_header(&mut src, 11);
    assert_eq!(top_err, 70, "no mirror + dead owner → 70");
    assert_eq!(echo, sid);
}
...
async fn wrong_epoch_after_promote_is_71() {
    ...
    produce_one(&leader_kafka, "ep", b"a").await;
    catch_up_isr(&leader, "ep");

    let body = fetch_v12("ep", 1, 0, 0);
    let resp = kafka_rpc(
        &leader_kafka,
        encode_request_flexible(1, 12, 10, Some("c"), &body),
    )
