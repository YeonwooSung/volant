...
async fn api_versions_self_advertises_max_2() {
    ...
    assert_eq!(found.get(&18), Some(&(0, 2))); // ApiVersions classic max
...
async fn api_versions_v1_trailing_throttle() {
    ...
    let found = parse_api_keys(&mut src);
    assert!(found.len() >= 10);
    assert_eq!(found.get(&18), Some(&(0, 2)));
...
#[tokio::test]
async fn api_versions_v3_unsupported() {
    let dir = temp_dir("v3");
    let broker = Arc::new(Broker::new(StorageConfig {
        data_dir: dir.clone(),
        ..StorageConfig::default()
    ...
    let resp = rpc(&addr, encode_request(18, 3, 9, Some("c"), &[])).await;
    let mut src = resp.freeze();
