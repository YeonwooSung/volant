//! Phase 119: multi-broker fetch session handoff (owner-encoded id + transparent forward).
...
use volant_broker::kafka::codec::{
    ...
    get_string, put_bytes, put_compact_array_len, put_compact_string,
    put_empty_tag_buffer, put_nullable_string, put_string, skip_tag_buffer,
};
use volant_broker::kafka::fetch_session::{decode_session_owner, session_local_part};
use volant_broker::{
...
/// Advance follower LEOs so leader HWM catches LEO (multi-node acks=1 produce).
...
impl ClusterHarness {
    async fn boot() -> Self {
        ...
        let mk = |id: u32| {
            ...
            // Keep sessions alive for the test window.
...
/// Open session on leader; incremental on a non-owner peer succeeds via forward.
#[tokio::test]
async fn incremental_on_peer_forwards_to_owner() {
    let h = ClusterHarness::boot().await;
    h.b1.create_topic("orders", 1).unwrap();
    propagate(&[&h.b1, &h.b2, &h.b3], "orders").await;
    ...
    let peer_kafka = h.kafka_of(peer_id);

    produce_one(leader_kafka, "orders", b"a").await;
    catch_up_isr(leader, "orders"); // HWM = 1

    // Create session at log end on leader (seeds omit cache).
    ...
    assert!(sid > 0);
    assert_eq!(
        decode_session_owner(sid),
        Some(leader_id),
        "session_id must embed owner; sid={sid:#x}"
    );
    assert!(session_local_part(sid) > 0);
    assert_eq!(leader.fetch_sessions().active_count(), 1);
    assert_eq!(peer.fetch_sessions().active_count(), 0);

    let fwd_before = peer.fetch_sessions().forward_total();

    // Empty-topics incremental on peer → transparent forward to owner.
    ...
    assert_eq!(top_err, 0, "forwarded incremental must succeed");
    assert_eq!(echo, sid);
    // Omit-unchanged: no topics when HWM/LSO stable.
    let n_topics = get_compact_array_len(&mut src).unwrap().unwrap();
    assert_eq!(n_topics, 0, "omit-unchanged after forward");

    assert!(
        peer.fetch_sessions().forward_total() > fwd_before,
        "peer must record a successful forward"
    );
    // Owner advanced epoch; peer must not hold a clone.
    assert_eq!(peer.fetch_sessions().active_count(), 0);
    assert_eq!(leader.fetch_sessions().active_count(), 1);

    // Next epoch still works via forward (expected epoch 2).
    let body = fetch_v12_empty_topics(sid, 2);
    let resp = kafka_rpc(
        peer_kafka,
        encode_request_flexible(1, 12, 12, Some("c"), &body),
    )
    .await;
    let mut src = resp.freeze();
...
/// Wrong epoch via peer still returns 71 from the owner.
#[tokio::test]
async fn wrong_epoch_via_peer_is_71() {
    let h = ClusterHarness::boot().await;
    h.b1.create_topic("events", 1).unwrap();
    ...
    produce_one(leader_kafka, "events", b"x").await;

    let body = fetch_v12("events", 1, 0, 0);
    let resp = kafka_rpc(
        leader_kafka,
    ...
    .await;
    let mut src = resp.freeze();
    let (top_err, sid) = assert_flex_header(&mut src, 20);
    assert_eq!(top_err, 0);
    assert!(sid > 0);

    // Stale epoch (expected is 1 after create). Use non-INITIAL so we forward.
    ...
    .await;
    let mut src = resp.freeze();
    let (top_err, echo) = assert_flex_header(&mut src, 21);
    assert_eq!(top_err, 71, "INVALID_FETCH_SESSION_EPOCH");
    assert_eq!(echo, sid);
}

/// FINAL close via peer removes session on the owner.
#[tokio::test]
async fn final_close_via_peer_closes_owner() {
    let h = ClusterHarness::boot().await;
    h.b1.create_topic("logs", 1).unwrap();
    propagate(&[&h.b1, &h.b2, &h.b3], "logs").await;
    ...
    produce_one(leader_kafka, "logs", b"z").await;

    let body = fetch_v12("logs", 1, 0, 0);
    let resp = kafka_rpc(
        leader_kafka,
    ...
    // FINAL_EPOCH = -1 via peer.
    let body = fetch_v12("logs", 0, sid, -1);
    let resp = kafka_rpc(
        peer_kafka,
    ...
    .await;
    let mut src = resp.freeze();
    let (top_err, _echo) = assert_flex_header(&mut src, 31);
    assert_eq!(top_err, 0);
    assert_eq!(
        leader.fetch_sessions().active_count(),
        0,
        "FINAL via peer must close owner session"
    );

    // Subsequent incremental → 70.
    let body = fetch_v12_empty_topics(sid, 1);
    let resp = kafka_rpc(
        peer_kafka,
    ...
    .await;
    let mut src = resp.freeze();
    let (top_err, echo) = assert_flex_header(&mut src, 32);
    assert_eq!(top_err, 70, "session gone after FINAL");
    assert_eq!(echo, sid);
}

/// Single-node still uses sequential (unencoded) session ids.
#[tokio::test]
async fn single_node_sequential_session_ids() {
    ...
    produce_one(&addr, "t", b"1").await;
    let body = fetch_v12("t", 1, 0, 0);
    let resp = kafka_rpc(
        &addr,
        encode_request_flexible(1, 12, 1, Some("c"), &body),
    )
    .await;
    let mut src = resp.freeze();
    let (top_err, sid) = assert_flex_header(&mut src, 1);
    assert_eq!(top_err, 0);
    assert!(sid > 0);
    assert_eq!(
        decode_session_owner(sid),
        None,
        "single-node ids must not embed owner"
    );
}
