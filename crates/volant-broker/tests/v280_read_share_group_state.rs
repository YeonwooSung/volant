...
#[tokio::test]
async fn api_versions_lists_read_share_group_state_84() {
    let (_dir, broker) = broker_temp("v280", "api");
    ...
    assert!(found.len() >= 80);
    assert_eq!(found.get(&84), Some(&(0, 0)));
...
#[tokio::test]
async fn read_share_group_state_v0_is_42_nothing_persisted() {
    let (_dir, broker) = broker_temp("v280", "reject");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;
    ...
    let resp = rpc(
        &addr,
        encode_request_flexible(84, 0, 10, Some("c"), &rsgs_v0_body("sg-v280", &topic_id, 0)),
    )
    .await;
    let mut src = resp.freeze();
    ...
    assert_eq!(src.get_i32(), 0); // StateEpoch
    assert_eq!(src.get_i64(), -1); // StartOffset
    let batches = get_compact_array_len(&mut src).unwrap().unwrap();
    assert_eq!(batches, 0);
    ...
    let offsets = broker.groups().fetch_offsets("sg-v280", &[]).unwrap();
    assert!(
        offsets.entries.is_empty(),
        "ReadShareGroupState must not persist offsets/share state"
    );
...
#[tokio::test]
async fn read_share_group_state_v1_is_35() {
    let (_dir, broker) = broker_temp("v280", "v1");
    let (addr, server) = boot_kafka(Arc::clone(&broker)).await;

    let resp = rpc(&addr, encode_request_flexible(84, 1, 99, Some("c"), &[])).await;
    let mut src = resp.freeze();
...
#[tokio::test]
async fn read_share_group_state_acl_deny_is_30() {
    let (_dir, broker) = broker_temp("v280", "acl");
    broker
    ...
    let resp = rpc(
        &addr,
        encode_request_flexible(84, 0, 11, Some("c"), &rsgs_v0_body("sg-v280", &topic_id, 2)),
    )
    .await;
    let mut src = resp.freeze();
