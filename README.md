...
> Status: **Phases 0–142 landed** (141 may ship as sibling N=2 tooling) — durable log, clustering (ISR death + rejoin /
> lag shrink + time-based ISR lag + Metadata leader ISR overlay + leader→controller IsrUpdate +
> PreferredReadReplica rack-aware Fetch +
...
> mirror coalesce/debounce + optional durable peer mirrors +
...
> multi-broker owner forward + peer mirror MVP (polish 139);
...
time-based ISR lag shrink via `replica_lag_max_ms` (Phase 125); PreferredReadReplica max LEO lag + RC suppress metric (Phase 140); Metadata leader ISR overlay + leader→controller IsrUpdate (Phase 142).
...
selector beyond 126/133/140 lag+metric (PreferredReadReplica MVP → **Phase 126**;
shared session mirror MVP → **Phase 138/139** — residual Raft registry /
serve-without-promote / incremental put), N=2 majority ops tooling, full KIP-890/939. Cluster admin fan-out →
...
Metadata ISR freshness → **Phase 142**.
