...
> Status: **Phases 0–103 landed** — durable log, clustering, security, stream
...
| Kafka wire shim | 23–103 | Optional `--kafka-listen`; classic + flexible; **~38** keys; prepared 2PC (**90**) + timeouts (**92**/open **93**/max **96**); omit-unchanged sessions (**91**); session TTL/max (**95**); bg sweeper + metrics (**97**/always-spawn **101**); crash≡abort control batches (**98**); broker Describe/AlterConfigs knobs (**99**) + sparse durable restart (**100**/ **102**) + name vs `node_id` (**103**) |
...
control markers, marker GC, multi-broker BROKER config fan-out, graceful sweeper join on stop.
