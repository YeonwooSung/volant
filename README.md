...
> Status: **Phases 0–94 landed** — durable log, clustering, security, stream
...
> Single-node mode (no `--cluster-config`) preserves the simple path. Start
> with the [whitepaper](./docs/WHITEPAPER.md) and [docs index](./docs/INDEX.md);
> also [ROADMAP.md](./ROADMAP.md), [ops](./docs/ops.md), [deploy/](./deploy/),
...
| Kafka wire shim | 23–93 | Optional `--kafka-listen`; classic + flexible; **~38** keys; prepared 2PC (**90**) + timeouts (**92**/open **93**); omit-unchanged sessions (**91**) |
...
READ_COMMITTED MVP (Phase 86); durable OffsetForLeaderEpoch history (Phase 87); Fetch DivergingEpoch + real fetch sessions MVP (Phase 88); Kafka control batches on EndTxn (Phase 89); prepared 2PC MVP (Phase 90); omit-unchanged incremental sessions (Phase 91); prepared timeout auto-abort (Phase 92); open-txn timeout (Phase 93).
