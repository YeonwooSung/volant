...
> Status: **Phases 0–142 + 144 landed** — durable log, clustering (ISR death + rejoin /
...
> preferred × established-session suppress +
...
time-based ISR lag shrink via `replica_lag_max_ms` (Phase 125); PreferredReadReplica max LEO lag + RC suppress metric (Phase 140); journal majority health gauges (Phase 141); Metadata leader ISR overlay + leader→controller IsrUpdate (Phase 142); preferred × established-session suppress (Phase 144).
...
serve-without-promote / incremental put / promote claim fence), full KIP-890/939. Cluster admin fan-out →
