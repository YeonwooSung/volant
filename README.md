...
> Status: **Phases 0–101 landed** — durable log, clustering, security, stream
...
> knobs with durable restart restore; sweeper always-spawn 0→>0 without restart).
...
| Kafka wire shim | 23–101 | Optional `--kafka-listen`; classic + flexible; **~38** keys; prepared 2PC (**90**) + timeouts (**92**/open **93**/max **96**); omit-unchanged sessions (**91**); session TTL/max (**95**); bg sweeper + metrics (**97**/always-spawn **101**); crash≡abort control batches (**98**); broker Describe/AlterConfigs knobs (**99**) + durable restart (**100**) |
...
control markers, marker GC, sparse durable config, BROKER name=`node_id`.
