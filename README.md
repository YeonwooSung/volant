...
> Status: **Phases 0–95 landed** — durable log, clustering, security, stream
...
> fetch session TTL/max). Single-node mode (no `--cluster-config`) preserves
> the simple path. Start with the [whitepaper](./docs/WHITEPAPER.md) and
...
| Kafka wire shim | 23–96 | Optional `--kafka-listen`; classic + flexible; **~38** keys; prepared 2PC (**90**) + timeouts (**92**/open **93**/max **96**); omit-unchanged sessions (**91**); session TTL/max (**95**) |
