...
| 63 | BrokerRegistration | 0 | Always flex; not KRaft (no incarnation/DirectoryId/features); parse and reject (**42**); does not wrap AddBroker; overlay unchanged; Cluster ALTER; controller not required (v0.263) |
...
| ACLs | LITERAL only; host always `*`; User resource (v3) stored only (no SCRAM-admin gating); **TransactionalId** (Kafka **5** / native **4**) consulted as Write on txn APIs when ACLs are on and `transactional_id` is non-empty (v0.247); no DelegationToken resource type; **CreateDelegationToken** (key **38**) advertised as a Cluster ALTER reject (v0.258); **DescribeDelegationToken** (key **41**) advertised empty (v0.259); Expire/Renew not advertised; cluster Create/Delete are **controller-only** with generationed snapshot fan-out (Phase 113) + durable-gen rejoin catch-up (Phase 117) — not Raft multi-master consensus |
...
| BrokerRegistration | **v0 reject** (v0.263): key **63** advertised (always flex). Not KRaft BrokerRegistration (no incarnation / DirectoryId / features). Parse official v0 body and reject **42** `INVALID_REQUEST`. Does **not** wrap native AddBroker. Overlay `membership.json` / broker list unchanged (`brokerEpoch` **-1**). Controller not required. ACL Cluster ALTER (deny → **31**). v1+ → **35**. Official Apache Kafka apiKey is **62**; this residual advertises **63**. |
...
| CreateDelegationToken | **v0 reject** (v0.258): key **38** advertised (always flex). No delegation-token store. Parse `renewers[]` + `maxLifeTimeMs`; top-level **42** `INVALID_REQUEST` (`delegation tokens not supported`). Empty/zero token fields (`tokenId` empty, `hmac` empty, issue/expiry/max **-1**). Nothing persisted. Controller not required. Official Kafka first flexible is **2**; Volant v0 is flexible. ACL Cluster ALTER (deny → **31**). v1+ → **35**. Expire/Renew not advertised |
| DescribeDelegationToken | **v0 empty** (v0.259): key **41** advertised (always flex residual; official first flex is **2**). No delegation-token store. Parse owners filter (null = all); error **0**, empty tokens. Official response order: `errorCode`, `tokens[]`, `throttleTimeMs` (no errorMessage). Nothing persisted. Controller not required. Cluster DESCRIBE. Denied → **31**, empty tokens. v1+ → **35**. Expire/Renew not advertised |
