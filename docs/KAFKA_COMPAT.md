...
| ACLs | LITERAL only; host always `*`; User resource (v3) stored only (no SCRAM-admin gating); **TransactionalId** (Kafka **5** / native **4**) consulted as Write on txn APIs when ACLs are on and `transactional_id` is non-empty (v0.247); DescribeDelegationToken **41** advertised empty (no token store; v0.259); Create/Expire/Renew not advertised; cluster Create/Delete are **controller-only** with generationed snapshot fan-out (Phase 113) + durable-gen rejoin catch-up (Phase 117) — not Raft multi-master consensus |
...
| DescribeDelegationToken | **v0 empty** (v0.259): key **41** advertised (always flex residual; official first flex is **2**). No delegation-token store. Parse owners filter (null = all); error **0**, empty tokens. Official response order: `errorCode`, `tokens[]`, `throttleTimeMs` (no errorMessage). Nothing persisted. Controller not required. Cluster DESCRIBE. Denied → **31**, empty tokens. v1+ → **35**. Create/Expire/Renew not advertised |
