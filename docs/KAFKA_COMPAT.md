...
| 1 | Fetch | 0–18 | Classic 0–11; flex v12–18; TopicId v13+; isolation LSO/aborted (Phase 86); ReplicaState v15+ (ReplicaId gates preferred redirect); NodeEndpoints v16+; CurrentLeader tag v12+; DivergingEpoch + sessions (Phase 88); omit-unchanged incremental (Phase 91); session idle TTL + max/LRU (Phase 95); durable local sessions (Phase 115); multi-broker session forward (Phase 119); best-effort peer mirror + promote on owner death (Phase 138; not Raft) + coalesce/debounce + optional durable mirrors (Phase 139); PreferredReadReplica 126+133+140 |
...
| Session mirror polish | 139 | Coalesce dirty ops (one per session_id; Delete supersedes Put); debounced Puts (`VOLANT_FETCH_SESSION_MIRROR_PUT_MIN_INTERVAL_MS` default 50; Delete immediate); optional durable `__fetch_session_mirrors` (`VOLANT_FETCH_SESSION_MIRROR_DURABLE`); `mirror_gen` fence; still not Raft / dual-promote residual |
