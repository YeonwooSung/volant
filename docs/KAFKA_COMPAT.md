...
| ACLs | LITERAL only; host always `*`; User resource (v3) stored only (no SCRAM-admin gating); **TransactionalId** (Kafka **5** / native **4**) consulted as Write on txn APIs when ACLs are on and `transactional_id` is non-empty (v0.247); no DelegationToken resource type; **CreateDelegationToken** (key **38**) advertised as a Cluster ALTER reject (v0.258); Describe/Expire/Renew not advertised; cluster Create/Delete are **controller-only** with generationed snapshot fan-out (Phase 113) + durable-gen rejoin catch-up (Phase 117) — not Raft multi-master consensus |
...
| CreateDelegationToken | **v0 reject** (v0.258): key **38** advertised (always flex). No delegation-token store. Parse `renewers[]` + `maxLifeTimeMs`; top-level **42** `INVALID_REQUEST` (`delegation tokens not supported`). Empty/zero token fields (`tokenId` empty, `hmac` empty, issue/expiry/max **-1**). Nothing persisted. Controller not required. Official Kafka first flexible is **2**; Volant v0 is flexible. ACL Cluster ALTER (deny → **31**). v1+ → **35**. Describe/Expire/Renew not advertised |
