...
| CreateDelegationToken | **v0 reject** (v0.258): key **38** advertised (always flex). No delegation-token store. Parse `renewers[]` + `maxLifeTimeMs`; top-level **42** `INVALID_REQUEST` (`delegation tokens not supported`). Empty/zero token fields (`tokenId` empty, `hmac` empty, issue/expiry/max **-1**). Nothing persisted. Controller not required. Official Kafka first flexible is **2**; Volant v0 is flexible. ACL Cluster ALTER (deny → **31**). v1+ → **35**. Expire not advertised |
| RenewDelegationToken | **v0 reject** (v0.261): key **39** advertised (always flex). No delegation-token store. Parse compact `hmac` + `renewPeriodMs`; top-level **42** `INVALID_REQUEST` (`delegation tokens not supported`). Official response: `errorCode`, `expiryTimestampMs` **-1**, `throttleTimeMs` **0** (no errorMessage). Nothing persisted. Controller not required. Official Kafka first flexible is **2**; Volant v0 is flexible. ACL Cluster ALTER (deny → **31**). v1+ → **35**. Expire not advertised |
| DescribeDelegationToken | **v0 empty** (v0.259): key **41** advertised (always flex residual; official first flex is **2**). No delegation-token store. Parse owners filter (null = all); error **0**, empty tokens. Official response order: `errorCode`, `tokens[]`, `throttleTimeMs` (no errorMessage). Nothing persisted. Controller not required. Cluster DESCRIBE. Denied → **31**, empty tokens. v1+ → **35**. Expire not advertised |
