...
- **Multi-broker 2PC (Phase 114 + 120):** Enable2Pc EndTxn prepare/complete fans out to live peers; local `__txn_prepared` + controller `__txn_prepared/cluster.json`; fence complete-abort with `commit=false`; **EndTxn** to non-coordinator transparent-forwards to Init owner (Phase 120); not full KIP-890/939
