...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–142 (shipped) |
| [PHASE7_SPEC.md](./PHASE7_SPEC.md) … [PHASE142_SPEC.md](./PHASE142_SPEC.md) | Per-phase ship records (deep dive) |
...
## Compaction note (2026-08-13, post–Phase 144 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **144**):

- **Status ceiling:** Phases **0–142, 144** shipped (143 may ship via sibling). Kafka shim **23–109** (… **135 = optional DeleteRecords majority wait**; **136 = non-blocking admin catch-up**; **137 = native DeleteRecords wait trailer + journal topic GC** — Kafka still env-only for wait; **138 = best-effort shared fetch session mirror + promote**; **139 = mirror coalesce/debounce + optional durable + `mirror_gen` fence**; **140 = preferred max LEO lag + RC suppress metric**; **141 = N=2 majority health gauges** `volant_cluster_*`; **142 = Metadata leader ISR overlay + IsrUpdate 94/95**; **144 = preferred × established-session suppress**)
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–144 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
