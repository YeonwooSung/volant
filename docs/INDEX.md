...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–135 (shipped) |
...
## Compaction note (2026-07-25, post–Phase 131 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **131**):

- **Status ceiling:** Phases **0–135** shipped. Kafka shim **23–109** (107 = test isolation; 108 = ISR death; 109 = accept drain; 110 = alive-set auto-death; 111 = straddle marker clip; 112 = fuzz corpus smoke CI; 113 = cluster admin fan-out; 114 = multi-broker 2PC MVP; 115 = durable fetch sessions; 116 = durable DeleteRecords outbox; 117 = ACL/BROKER admin catch-up; 118 = ISR rejoin + lag shrink; 119 = multi-broker fetch session handoff; 120 = transparent EndTxn forward; 121 = sticky FindCoordinator; 122 = AddOffsets / TxnOffsetCommit forward; 123 = DeleteRecords outbox leadership handoff; 124 = durable txn coordinator registry; 125 = time-based ISR lag shrink; 126 = PreferredReadReplica / rack-aware Fetch MVP; 127 = txn coordinator registry TTL GC; 128 = BROKER config for registry TTL; 129 = truncate journal; 130 = multi-controller majority journal; 131 = truncate journal rejoin catch-up; 132 = journal catch-up hardening; 133 = preferred selector polish; 134 = p2p heartbeat mesh; **135 = optional DeleteRecords majority wait**)
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–135 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
...
- **PreferredReadReplica (Phase 126):** Fetch v11+ client rack; leader redirects to same-rack ISR peer with LEO≥HWM (empty records); Metadata rack from `cluster.toml`; not full Kafka selector/throttling; not shared session store
- **Fuzz / CI (Phase 112):** deterministic corpus smoke + `.github/workflows/ci.yml`; long campaigns / chaos-mesh still deferred
- **Still deferred (product):** multi-lang, chaos-mesh / long fuzz campaigns, shared session store / full preferred selector, full KIP-890/939 / `__transaction_state` topic; full Kafka broker catalog
