...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–138 (shipped) |
...
## Compaction note (2026-08-12, post–Phase 138 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **138**):

- **Status ceiling:** Phases **0–138** shipped. Kafka shim **23–109** (… **135 = optional DeleteRecords majority wait**; **136 = non-blocking admin catch-up**; **137 = native DeleteRecords wait trailer + journal topic GC** — Kafka still env-only for wait; **138 = best-effort shared fetch session mirror + promote**)
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–138 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
...
- **Fetch sessions (shipped MVP):** create/forgotten/errors; omit-unchanged empty-topics incremental (Phase 91); idle TTL + max/LRU (Phase 95); background idle sweep (Phase 97/101/106); BROKER config surface (Phase 99–103 sparse durable + name validation; **cluster fan-out** Phase 113); **durable per-broker table** under `__fetch_sessions` (Phase 115); **multi-broker owner-encode + transparent forward** (Phase 119); **best-effort peer mirror + promote-on-owner-miss** (Phase 138; opcodes 90–93; not Raft; dual-promote race honest)
- **PreferredReadReplica (Phase 126):** Fetch v11+ client rack; leader redirects to same-rack ISR peer with LEO≥HWM (empty records); Metadata rack from `cluster.toml`; not full Kafka selector/throttling; preferred still orthogonal to session mirror
- **Fuzz / CI (Phase 112):** deterministic corpus smoke + `.github/workflows/ci.yml`; long campaigns / chaos-mesh still deferred
- **Still deferred (product):** multi-lang, chaos-mesh / long fuzz campaigns, full preferred selector (beyond 133), Raft session registry / serve-from-mirror-without-promote residual, full KIP-890/939 / `__transaction_state` topic; full Kafka broker catalog
