...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–105 |
| [PHASE7_SPEC.md](./PHASE7_SPEC.md) … [PHASE105_SPEC.md](./PHASE105_SPEC.md) | Per-phase ship records (deep dive) |
...
## Compaction note (2026-07-18, post–Phase 105 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **105**):

- **Status ceiling:** Phases **0–105**; Kafka shim **23–105**
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–105 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
- **README / ops:** compact bands + ops table (not per-phase diaries)
- **Txn honesty (shipped):** write-through + soft markers + EndTxn control batches (Phase 89) + crash-promote ABORT control (Phase 98) + **empty AddPartitions control** (Phase 105) + prepared 2PC MVP (Phase 90) + prepared/open timeout (Phase 92/93) + TRANSACTION_ABORTABLE honest subset after timeout (Phase 94) + transaction max timeout clamp (Phase 96; default 15m; Init **50** over-max) + background sweeper (Phase 97; always-spawn / 0→>0 live Phase 101) + BROKER Describe/AlterConfigs knobs (Phase 99) + **sparse** durable restart restore (Phase 100/102) + BROKER name vs local `node_id` (Phase 103) + **aborted soft-marker GC** on DeleteRecords/retention/load (Phase 104)
...
- **Still deferred (product):** multi-lang, chaos/fuzz corpus CI, multi-broker 2PC /
  ...
  full Kafka broker catalog; graceful sweeper join on stop
