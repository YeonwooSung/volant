...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–126 |
...
## Compaction note (2026-07-25, post–Phase 126 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **126**):

- **Status ceiling:** Phases **0–126**; Kafka shim **23–109** (107 = test isolation; 108 = ISR death; 109 = accept drain; 110 = alive-set auto-death; 111 = straddle marker clip; 112 = fuzz corpus smoke CI; 113 = cluster admin fan-out; 114 = multi-broker 2PC MVP; 115 = durable fetch sessions; 116 = durable DeleteRecords outbox; 117 = ACL/BROKER admin catch-up; 118 = ISR rejoin + lag shrink; 119 = multi-broker fetch session handoff; 120 = transparent EndTxn forward; 121 = sticky FindCoordinator; 122 = AddOffsets / TxnOffsetCommit forward; 123 = DeleteRecords outbox leadership handoff; 124 = durable txn coordinator registry; 125 = time-based ISR lag shrink; **126 = PreferredReadReplica / rack-aware Fetch MVP**)
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–126 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
...
- **Multi-broker 2PC (Phase 114 + 120 + 121 + 122 + 124):** Enable2Pc EndTxn prepare/complete fans out to live peers; local `__txn_prepared` + controller `__txn_prepared/cluster.json`; fence complete-abort with `commit=false`; **EndTxn / AddOffsets / TxnOffsetCommit** to non-coordinator transparent-forwards to Init owner (Phase 120/122); **FindCoordinator** sticky murmur2 + Init-owner override (Phase 121); **Init-owner registry durable** under `__txn_coordinator` (Phase 124); not full KIP-890/939
...
- **Still deferred (product):** multi-lang, chaos-mesh / long fuzz campaigns, shared session store / full preferred selector, full KIP-890/939 / `__transaction_state` topic; full Kafka broker catalog; registry GC/TTL
