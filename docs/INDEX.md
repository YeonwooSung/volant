...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–137 (shipped) |
...
## Compaction note (2026-08-12, post–Phase 137 ship)

Living docs match **git HEAD product** (`SUPPORTED_APIS`, last feature commit Phase **137**):

- **Status ceiling:** Phases **0–137** shipped. Kafka shim **23–109** (… **135 = optional DeleteRecords majority wait**; **136 = non-blocking admin catch-up**; **137 = native DeleteRecords wait trailer + journal topic GC** — Kafka still env-only for wait)
- **Kafka SoT:** [KAFKA_COMPAT.md](./KAFKA_COMPAT.md) — matrix + semantic honesty
- **WHITEPAPER:** architecture + positioning; no full API matrix
- **Binding core:** PHASE1–6; **ship records:** PHASE7–137 via [PHASE_HISTORY](./history/PHASE_HISTORY.md)
