...
| Document | Purpose |
|----------|---------|
| [history/PHASE_HISTORY.md](./history/PHASE_HISTORY.md) | One-line index for phases 0–121 |
| [PHASE7_SPEC.md](./PHASE7_SPEC.md) … [PHASE121_SPEC.md](./PHASE121_SPEC.md) | Per-phase ship records (deep dive) |
...
- **Multi-broker 2PC (Phase 114 + 120 + 121):** Enable2Pc EndTxn prepare/complete fans out to live peers; local `__txn_prepared` + controller `__txn_prepared/cluster.json`; fence complete-abort with `commit=false`; **EndTxn** to non-coordinator transparent-forwards to Init owner (Phase 120); **FindCoordinator** sticky murmur2 + Init-owner override (Phase 121); not full KIP-890/939
...
- **Still deferred (product):** multi-lang, chaos-mesh / long fuzz campaigns, preferred-replica / shared session store, full KIP-890/939 / `__transaction_state` topic; full Kafka broker catalog; remaining txn API forward (AddOffsets / TxnOffsetCommit)
