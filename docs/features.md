...
| Shared session mirror (Phase 138+139) | Best-effort peer put/delete (opcodes 90–93); foreign mirror table; promote on owner forward miss; **139:** coalesce dirty ops, debounced Puts (`VOLANT_FETCH_SESSION_MIRROR_PUT_MIN_INTERVAL_MS` default 50; Delete immediate), optional durable `__fetch_session_mirrors` (`VOLANT_FETCH_SESSION_MIRROR_DURABLE`), `mirror_gen` fence; **not** Raft; dual-promote race may **71** |
...
## Fetch DivergingEpoch + sessions (Phase 88 + 91 + 95 + 115 + 119 + 138/139) + preferred replica (126+133+140+144)
...
| Shared mirror (138+139) | Best-effort MirrorPut/Delete (90–93) after primary mutations; foreign mirror not served while owner alive; owner forward fail → `promote_from_mirror` then local serve; put lag/fail still **70**; dual-promote race may later **71**; **139:** one pending op per `session_id` (Delete supersedes Put); debounced Puts (`VOLANT_FETCH_SESSION_MIRROR_PUT_MIN_INTERVAL_MS` default 50; Delete immediate); optional durable `__fetch_session_mirrors/state.json` (`VOLANT_FETCH_SESSION_MIRROR_DURABLE`); `mirror_gen` fencing; metrics `volant_fetch_session_mirror_puts_total` / `_deletes_total` / `volant_fetch_session_promote_total` / `volant_fetch_sessions_mirrored` + `_puts_coalesced_total` / `_stale_put_rejects_total` / `volant_fetch_session_promote_supersede_total` / `volant_fetch_session_mirror_restored` |
| PreferredReadReplica (126+133+140+144) | Fetch v11+ client `rack_id`; leader may set PreferredReadReplica to same-rack live ISR peer with usable addr + LEO≥HWM (empty records redirect); **rank highest LEO then lowest id** (Phase 133); optional max lag vs leader LEO (`VOLANT_PREFERRED_REPLICA_MAX_LEO_LAG`; unset = unlimited) (Phase 140); **suppressed when isolation=READ_COMMITTED** (leader serves aborted-marker filter; metric `volant_preferred_replica_suppressed_total` when a candidate existed) (Phase 140); **suppressed when client has established fetch session** (`req_session_id != 0`, non-FINAL; metric `volant_preferred_replica_session_suppressed_total`) so session-on-leader then preferred does not thrash via forward/promote (Phase 144); full fetch `session_id == 0` may still redirect; gated off for followers (`replica_id` top-level ≤v14 / ReplicaState tag 1 on v15+); Metadata/DescribeCluster/NodeEndpoints advertise `cluster.toml` rack; metric `volant_preferred_replica_redirect_total` |
...
| Feature | Behavior |
|---------|----------|
| DeleteRecords fan-out | Partition **leader** truncates locally, then best-effort `ReplicaDeleteRecords` / journal note / outbox at **achieved** `low_watermark` after whole-segment clamp (not client-requested `before_offset`; soft-marker GC/clip on peers). **Default:** client success does **not** wait on peer/journal majority. Metric `volant_delete_records_fanout_errors_total`. **Phase 116:** peers pre-enqueued under `{data_dir}/__delete_records_outbox` (at-least-once; drop on success) and drained when live. **Phase 123:** new leader reconciles pending truncates from local `log_start` after leadership change. **Phase 129/130:** truncate journal (`__truncate_journal`); multi-controller majority note (Raft-style) + **always full-snapshot** `TruncateJournalPush` to live peers; reconcile = `max(local log_start, journal watermark)`. **Phase 131:** heartbeat piggyback `applied_journal_generation` + lag-driven re-push so offline/rejoining peers converge without waiting for the next DeleteRecords. **Phase 132:** catch-up is **non-blocking** (schedule + spawn from HeartbeatBroker; per-peer single-flight + min-interval throttle via `VOLANT_JOURNAL_CATCHUP_MIN_INTERVAL_MS`, default 500ms) so membership is not stalled by full-snapshot push. **Phase 134:** peer-to-peer heartbeat mesh (each broker HB all configured peers; alive-set apply only vs controller) so non-controller journal holders can catch up lagging peers without the controller holding that watermark. **Phase 135 (opt-in):** `VOLANT_DELETE_RECORDS_WAIT_MAJORITY=1|true|yes` makes the client path await journal majority; fail → native `NotEnoughReplicas` (**15**) / Kafka **19**; local low still returned (no rollback); metrics `volant_delete_records_majority_wait_success_total` / `_fail_total`. **Phase 137:** native optional trailer `wait_majority` u8 after `before_offset` (`0`=broker default, `1`=force wait, `2`=force no-wait); `Client::delete_records_with_wait_flag` + CLI `--wait-majority` / `--no-wait-majority`; Kafka still env/broker only. **Phase 137 journal GC:** `apply_cluster_state` prunes removed topics from the journal; push apply skips entries whose topic is not in the known assignment/local set (anti-resurrection). Caps: 100k entries / 4 MiB push snapshot; local `delete_topic` also prunes journal watermarks (best-effort, no gen bump); metrics `volant_journal_catchup_success_total` / `_errors_total` / `_skipped_total` |
