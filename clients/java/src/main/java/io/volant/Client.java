...
public final class Client implements AutoCloseable {
    ...
    /**
     * Produce one message with a caller-supplied native timestamp and
     * native record headers, using the client default acks. Named so it
     * does not collide with {@link #produce(String, int, byte[], byte[], int)}
     * or {@link #produce(String, int, byte[], byte[], List)}.
     * {@link #produceTimestamp} still sends empty headers. The headers
     * {@code produce} and {@link #produceHeadersAcks} still send
     * {@code timestampMs = -1}. Reuses the batch retry / error 13 /
     * error 21 path.
     */
    public long produceTimestampHeaders(
            String topic, int partition, byte[] key, byte[] value, long timestampMs, List<Record.Header> headers) {
        if (value == null) {
            value = new byte[0];
        }
        return produce(
                topic,
                partition,
                Collections.singletonList(new Codec.ProduceMessage(key, value, timestampMs, headers)),
                this.acks);
    }

    /**
     * Produce one message with a caller-supplied native timestamp, empty
     * headers, and an explicit acks byte. {@code 1} = leader only;
     * {@code 255} = acks=all (ISR). Named so it does not collide with
     * existing {@code produce} overloads. {@link #produceTimestamp} still
     * uses {@link #acks()}. The acks {@code produce} still sends
     * {@code timestampMs = -1}. Reuses the batch retry / error 13 /
     * error 21 path.
     */
    public long produceTimestampAcks(
            String topic, int partition, byte[] key, byte[] value, long timestampMs, int acks) {
        if (value == null) {
            value = new byte[0];
        }
        return produce(
    ...
    /**
     * Produce one message with a caller-supplied native timestamp,
     * native record headers, and an explicit acks byte. {@code 1} =
     * leader only; {@code 255} = acks=all (ISR). Named so it does not
     * collide with existing {@code produce} overloads.
     * {@link #produceTimestampHeaders} still uses {@link #acks()}.
     * {@link #produceHeadersAcks} still sends {@code timestampMs = -1}.
     * Reuses the batch retry / error 13 / error 21 path.
     */
    public long produceTimestampHeadersAcks(
            String topic,
            int partition,
            byte[] key,
            byte[] value,
