...
class JoinGroupInstanceTest {
    @Test
    void joinGroupSendsEmptyInstanceId() throws Exception {
    ...
    @Test
    void joinGroupWithInstanceEncodesId() throws Exception {
        try (JoinGroupStub srv = new JoinGroupStub()) {
            try (Client c = Client.connect("127.0.0.1", srv.port, 5_000)) {
                JoinGroupResult j = c.joinGroupWithInstance("g", List.of("t"), 10_000, "pod-1");
                assertEquals("m-1", j.memberId);
            }
            srv.assertOk();
            assertEquals("pod-1", srv.instanceId.get());
        }
    }

    @Test
    void joinGroupWithInstanceEmptyMatchesPublicApi() throws Exception {
        try (JoinGroupStub srv = new JoinGroupStub()) {
            try (Client c = Client.connect("127.0.0.1", srv.port, 5_000)) {
                c.joinGroupWithInstance("g", List.of("t"), 10_000, "");
            }
            srv.assertOk();
            assertEquals("", srv.instanceId.get());
        }
