...
try (Client c = Client.connect("127.0.0.1", 9092)) {
  c.createTopic("t", 1);
  // createTopic(name) is createTopic(name, 1) (v0.199). 2-arg stays explicit.
  c.createTopic("t");
  ...
  c.alterConfig("t", "retention.ms", "86400000"); // v0.177; one key
...
`alterConfigs` with a singleton list.
