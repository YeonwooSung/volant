...
JoinGroupConsumer retries Join on generation-fence error 9 with `SetMaxRetries` /
`SetRetryBackoff` (default 0 extra attempts; concurrent joins need
`SetMaxRetries` > 0).
...
same `SetMaxRetries` / `SetRetryBackoff` as other Join retries
(default 0).
...
`DialTLS` are unchanged. `AuthToken()` (v0.200) returns the stored
shared-token used for opcode 30, or empty if none; it does not send
Auth.
