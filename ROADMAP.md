...
- [ ] Multi-language clients (Rust first; Go / Python FFI or REST gateway) — **deferred** (Python/Go/Java through v0.175; Rust add_broker_no_rack **v0.172**)
...
membership / Raft, full Kafka broker catalog, consensus truncate journal /
controller SoT pending set, per-broker BROKER config overrides, shared
session registry, full preferred-replica selector parity,
registry GC/TTL → **closed by Phase 127**.

---

### Phase 127 — Txn coordinator registry TTL GC (MVP) ✅

**Goal:** Expire stale Phase 124 Init-owner registry entries after a configurable
TTL so completed/abandoned transactional_id and producer_id mappings do not
grow without bound.

Binding: **[docs/PHASE127_SPEC.md](./docs/PHASE127_SPEC.md)**.

- [x] Durable last-touch ms maps (file format v2; v1 load → now timestamps)
- [x] `expire_stale(ttl, now)` + persist; metric `volant_txn_coordinator_registry_gc_total`
- [x] `VOLANT_TXN_COORDINATOR_TTL_MS` (default 24h; `0` disables)
- [x] Hook into `sweep_timeouts` background path
- [x] Tests: unit + `phase127_txn_coordinator_gc`
- [x] Living docs honesty

**Honest limitations:** wall-clock TTL; long-lived txns must re-note within TTL;
no eager EndTxn GC; env-only (not BROKER Describe/Alter); not cluster SoT.
...
heartbeat journal rejoin catch-up → **closed by Phase 131**.

---

### Phase 131 — Truncate journal rejoin catch-up (MVP) ✅

**Goal:** Peers that miss Phase 129/130 journal note/push (offline, flaky RPC,
restart) **converge** after rejoin via heartbeat lag detection + best-effort
`TruncateJournalPush`, instead of permanent watermark drift until the next
DeleteRecords.

Binding: **[docs/PHASE131_SPEC.md](./docs/PHASE131_SPEC.md)**.

- [x] `HeartbeatBroker` trailer `applied_journal_generation` (after config/ACL gens)
- [x] Lag-driven full-snapshot `TruncateJournalPush` (opcode 88); multi-controller ok
- [x] Metrics `volant_journal_catchup_success_total` / `_errors_total`
- [x] Tests: `phase131_journal_catchup` + protocol trailer round-trips
- [x] Living docs honesty
...
session_id owner bits; full snapshot put per mutation can be chatty (Phase 139
coalesce/debounce); preferred selector residual still orthogonal (Phase 126/133/140);
single-node no fan-out.

**Still deferred:** Raft / controller session registry; serve from mirror without
promote; incremental/delta MirrorPut wire; full preferred selector; rollback
...
**Goal:** Make Phase 138 peer mirrors less chatty and optionally durable, with
light `mirror_gen` fencing. No new opcodes; still **not** Raft.

Binding: **[docs/PHASE139_SPEC.md](./docs/PHASE139_SPEC.md)**.

- [x] Coalesce dirty ops (one pending op per `session_id`; Delete supersedes Put)
- [x] Debounced Puts: `VOLANT_FETCH_SESSION_MIRROR_PUT_MIN_INTERVAL_MS` default **50**;
  Delete flushes immediately
- [x] Optional durable mirrors: `VOLANT_FETCH_SESSION_MIRROR_DURABLE` →
  `{data_dir}/__fetch_session_mirrors/state.json` (default off; idle TTL filter on load)
- [x] `mirror_gen` fencing on `apply_mirror_put` / `promote_from_mirror`
- [x] Metrics: `volant_fetch_session_mirror_puts_coalesced_total`,
...
- [x] Tests phase139 unit + `phase139_session_mirror_polish` (+ phase138 green)
- [x] Living docs 0–139

**Honest limitations:** still best-effort (put lag/fail **70**); dual-promote race
may **71** until claim exchange (Phase **143**); no incremental/delta wire; no
serve-from-mirror without promote; durable mirrors off by default.

**Still deferred:** Raft / controller session registry; serve from mirror without
promote; incremental MirrorPut; full preferred selector; rollback local truncate
on majority fail; full openraft/KRaft; multi-lang; chaos/long fuzz; full KIP-890/939;
heterogeneous per-broker BROKER overrides.
...
**Goal:** Observability + optional freshness lag on Phase 126/133 preferred
selection. Does **not** re-ship usable-addr or LEO ranking (already 133).

Binding: **[docs/PHASE140_SPEC.md](./docs/PHASE140_SPEC.md)**.

- [x] Optional `VOLANT_PREFERRED_REPLICA_MAX_LEO_LAG` (parsed u64; **unset** =
...
(119/138/139); RC still forces leader serve for aborted-marker filter.

**Still deferred:** full preferred selector / throttling / assignment; serve from
mirror without promote; Raft session registry; rollback local truncate on majority
fail; full openraft/KRaft; multi-lang; chaos/long fuzz; full KIP-890/939;
heterogeneous per-broker BROKER overrides.
...
**Goal:** Dual-promote of equal-freshness mirrors converges to one primary via
lowest non-zero `promoted_by` after MirrorPut exchange.

Binding: **[docs/PHASE143_SPEC.md](./docs/PHASE143_SPEC.md)**.

- [x] `promoted_by` on `FetchSession` / durable + MirrorPut JSON (default `0`)
- [x] `session_claim_wins` after `mirror_gen`/epoch/activity
- [x] `promote_from_mirror` / `apply_mirror_put` claim-aware
- [x] Metric `volant_fetch_session_promote_claim_reject_total`
- [x] Tests `phase143_promote_claim_fence`; phase138/139 green

**Honest limitations:** best-effort MirrorPut exchange (brief dual primary until
exchange); not Raft session registry; no session_id re-encode.
...
90–93; promote on owner miss — not Raft) + **mirror polish** Phase 139
(coalesce/debounce Puts, optional durable `__fetch_session_mirrors`, `mirror_gen`
fence) + **promote claim fence** Phase 143 (`promoted_by` lowest-id; best-effort
MirrorPut; not Raft);
...
Still deferred: multi-language clients (Python/Go/Java through v0.175; Rust add_broker_no_rack **v0.172**), full chaos-mesh suites / long fuzz
