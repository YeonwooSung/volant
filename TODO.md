...
**Baseline:** HEAD product = **Phases 0–154** + residuals **v0.3–v0.65**.  
...
| **Later** | **Multi-language clients** | **Python/Go/Java through v0.65** + Rust auto-commit **v0.60**; not kafka-python / SyncGroup |
...
**Default next slice:** openraft RocksDB if redb is not enough, or SyncGroup (needs a member list — no new Kafka keys). Homemade Raft election / InstallSnapshot-on-154 / Phase 155 is **not** the next product bet. Do not open Phase 155.
...
- [x] Multi-language clients — Python/Go/Java through v0.65 (incl. produce retry, auto_offset_reset, TransactionalProducer, Fetch knobs, DeleteRecords redirect) + Rust auto-commit **v0.60**; not kafka-python / SyncGroup
...
- Produce retry (v0.61) is default **0**; transient codes 6/7/15/16 + TCP I/O only. Fetch is not retried
- Auto-commit is poll-tied and default **off** (language **v0.48**, Rust **v0.60**); not Kafka `enable.auto.commit`
- GroupConsumer `auto_offset_reset` (v0.62): `earliest` is **0** (no ListOffsets); `latest` is LEO. Not Kafka timestamp reset. Rust GroupConsumer still OffsetFetch / 0
