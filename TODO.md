...
**Baseline:** HEAD product = **Phases 0–154** + residuals **v0.3–v0.155**.  
...
| **Later** | **Multi-language clients** | **Python/Go/Java through v0.155** + Rust InitProducerId / OffsetCommit / OffsetFetch topic / DeleteRecords wait **v0.151–v0.155**; not kafka-python / SyncGroup |
...
**Default next slice:** JoinGroup is still not retried (not idempotent). Go `CreateTopic` still discards the topic id (`CreateTopicID` / WithConfigs return it), or openraft RocksDB if redb is not enough. SyncGroup still has no opcode. Homemade Raft election / InstallSnapshot-on-154 / Phase 155 is **not** the next product bet. Residual **v0.155** is DeleteRecords wait config, not Phase 155. Do not open Phase 155.
...
- [x] Multi-language clients — Python/Go/Java through v0.155 (incl. DeleteRecords default wait) + Rust InitProducerId / OffsetCommit helpers / OffsetFetch topic / DeleteRecords wait **v0.151–v0.155**; not kafka-python / SyncGroup
