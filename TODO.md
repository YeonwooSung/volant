...
**Baseline:** HEAD product = **Phases 0–154** + residuals **v0.3–v0.291**; **Phase 155 open**.  
...
**Default next slice:** Kafka `SUPPORTED_APIS` is **91** (EndQuorumEpoch **54**, StreamsGroupTopologyDescriptionUpdate **93**). Official leftover keys are exhausted (ApiKeys.java ends at UnregisterController **94**; removed **4/5/6/7** not advertised). Still not join-set wait, not unclean election, not live reassignment, not stored quotas, not KIP-584, not token store, not KIP-848, not KIP-932, not KIP-1071. Residual **v0.155** is still DeleteRecords wait.
...
- [x] delete homemade 154 hatch → **v0.222**
- [x] language Client Join 9 retry → **v0.223**
...
- [x] Kafka DescribeLogDirs key 35 → **v0.235**
- [x] Kafka ElectLeaders key 43 → **v0.236**
...
- [x] Kafka DescribeQuorum 55 → **v0.245**
- [x] Kafka AllocateProducerIds 67 → **v0.246**
- [x] ACL TransactionalId on txn APIs → **v0.247**
- [x] SyncGroup apply decoded assignment → **v0.248**
- [x] Kafka AlterReplicaLogDirs 34 reject → **v0.249**
- [x] Kafka WriteTxnMarkers 27 → **v0.250**
- [x] Kafka AssignReplicasToDirs 73 reject → **v0.251**
- [x] Kafka ListClientMetricsResources 74 empty → **v0.252**
- [x] Kafka GetTelemetrySubscriptions 71 empty → **v0.253**
- [x] TxnOffsetCommit v3+ generation/member fence → **v0.254**
- [x] Kafka PushTelemetry 72 reject → **v0.255**
- [x] OffsetFetch RequireStable honors LSO → **v0.256**
...
- [x] Kafka BrokerHeartbeat 63 reject → **v0.265**
- [x] Kafka Envelope 58 reject → **v0.266**
...
- [x] Kafka ShareGroupHeartbeat 76 reject → **v0.275**
- [x] Kafka ShareGroupDescribe 77 reject → **v0.276**
- [x] Kafka ShareFetch 78 reject → **v0.277**
- [x] Kafka ShareAcknowledge 79 reject → **v0.278**
- [x] Kafka InitializeShareGroupState 83 reject → **v0.279**
- [x] Kafka ReadShareGroupState 84 reject → **v0.280**
- [x] Kafka WriteShareGroupState 85 reject → **v0.281**
...
- [x] Kafka EndQuorumEpoch 54 reject → **v0.290**
- [x] Kafka StreamsGroupTopologyDescriptionUpdate 93 reject → **v0.291**
...
- [x] **InstallSnapshot** / log truncation on **openraft** (v0.17; homemade 154 deleted **v0.222**)
...
- Local `assignor="range"` uses JoinGroup members trailer when present (v0.211); empty trailer still DescribeGroup (language **v0.69**, Rust **v0.73**). SyncGroup is a generation confirm fence (v0.215), not parked Join
- Language-client SCRAM handshake is SHA-256 by default (v0.46); SHA-512 is opt-in trailer **2** (**v0.238**). Admin Create/Delete/ListScramUsers are native **64–69** (v0.55); password is sent in the clear on create (use TLS)
...
| v0.290–v0.291 | **Shipped** — EndQuorumEpoch 54 reject; StreamsGroupTopologyDescriptionUpdate 93 reject |
