...
**Baseline:** HEAD product = **Phases 0–154** + residuals **v0.3–v0.115**.  
...
| **Later** | **Multi-language clients** | **Python/Go/Java through v0.115** + Rust DeleteRecords 13+retry / ListOffsets 13 / metadata_topics **v0.111/v0.113/v0.114**; not kafka-python / SyncGroup |
...
**Default next slice:** JoinGroup is still not retried (not idempotent). Go/Java metadata topic filter (Rust **v0.114** / Python already), or openraft RocksDB if redb is not enough. SyncGroup still has no opcode. Homemade Raft election / InstallSnapshot-on-154 / Phase 155 is **not** the next product bet. Do not open Phase 155.
...
- [x] Multi-language clients — Python/Go/Java through v0.115 (incl. ListOffsets 13, public reconnect) + Rust DeleteRecords 13+retry / ListOffsets 13 / metadata_topics **v0.111/v0.113/v0.114**; not kafka-python / SyncGroup
