...
**Baseline:** HEAD product = **Phases 0–154** + residuals **v0.3–v0.200**.  
...
| **Later** | **Multi-language clients** | **Python/Go/Java through v0.200** + Rust metadata_topic **v0.182**; not kafka-python / SyncGroup |
...
**Default next slice:** JoinGroup is still not retried (not idempotent). Go `CreateTopic` still discards the topic id (`CreateTopicID` / WithConfigs / Java `createTopic` return it). Java `heartbeatIntervalMs` is still package-private. SCRAM username has no Go/Java getter (password stays private). SyncGroup still has no opcode. Homemade Raft election / InstallSnapshot-on-154 / Phase 155 is **not** the next product bet. Residual **v0.155** is DeleteRecords wait config, not Phase 155. Do not open Phase 155.
...
- [x] Multi-language clients — Python/Go/Java through v0.200 (incl. Python All-helpers, CreateTopic default partitions=1, Go/Java auth token getters) + Rust metadata_topic **v0.182**; not kafka-python / SyncGroup
